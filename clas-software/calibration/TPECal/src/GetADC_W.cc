...
GetADC_W :: GetADC_W(QWidget *mom, QSplashScreen* spl, QApplication *ecg, 
                     tpecal_opts *op, tpecal_calib *tec, TCanvas *Sp) : QWidget(mom)
{
	splash       = spl;
	tpecal_gui   = ecg;
	tpecalOpt    = op;
	...
	for(unsigned int i=0; i<tpecalOpt->ifiles.size(); i++)
		bosfiles->addItem(tpecalOpt->ifiles[i].c_str());
	...
	connect ( CANCEL_get_adc, SIGNAL( clicked() ), this, SLOT( cancelTPECal()            ) );
