...
proc checkIn {} {
    ...
    set subs(deltaT) "delta_T"
    set item(deltaT) "pd2pd"

    set subs(veffLg) "veff"
    set item(veffLg) "leg"

    set subs(veffNs) "veff"
    set item(veffNs) "nose"

    set subs(veffN1) "veff"
    set item(veffN1) "nose1"

    set subs(veffN2) "veff"
    set item(veffN2) "nose2"

    set ff [open $dummyrc w]
    puts $ff "#!/bin/sh"
    puts $ff "export PERL5LIB=$perl5lib"
    foreach tx {deltaT veffLg veffNs veffN1 veffN2} {
	foreach lr {left right} {
	    set fname "$constPath/STN_CALIB_$subs($tx)_$item($tx).dat"
	    
	    puts $ff "$perl5lib/$perlprog hostname=$caldb(host) \
                      user=$caldb(user) $passwd   \
                      s=STN_CALIB ss=$subs($tx) i=$item($tx) \
                      min=$caldb(runm) max=$caldb(rmax)   \
                      srmin=$caldb(rsrc) srmax=$caldb(rsrc)      \
...
proc searchPerlscript {} {
    global perl5lib perlprog
    set clas_root "/group/clas"
    catch {set clas_root  $env(CLAS_ROOT)}


    if [file exists "$perl5lib/$perlprog" ] {
	showForms
    } elseif [file exists "$clas_root/tools/caldb/$perlprog" ] {
	set perl5lib "$clas_root/tools/caldb"
	showForms
