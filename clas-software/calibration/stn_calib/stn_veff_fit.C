//$Id: stn_veff_fit.C,v 1.3 2008/03/27 14:22:50 pasyuk Exp $
#ifndef __CINT__
#include "ROOT.h"
#include<iostream>
#include<fstream>
...
void show_final_plots() {
  ...
  for (int i=0; i<cdb_max; i++) {
    char outfilename[80];
    sprintf (outfilename, "/tmp/STN_CALIB_%s.dat", caldb_table[i]);
...
void stn_veff_fit() {
  ...
  C = new TDialogCanvas ("C", "C", 800, 800);
  C->SetFillColor(42);
