...
TClonesArray *SCRCStore;
static TClonesArray *fgSCRCStore;
TClonesArray *DC0Store;
static TClonesArray *fgDC0Store;
	...
	cout << "Event " << iCount << " Ph Event: " << hHEADER->NEvent <<endl<<endl;
	 ...
	 for(int j = 0; j < kSCPB.get_nrows(); j++){
      ...
      //
       ...
       SCRC    kSCRC(0);
       TSCRCClass   *SCRC_p = new TSCRCClass();
       if(kSCRC.get_nrows() > 0){
	 for(int j = 0; j < kSCRC.get_nrows(); j++){
	   Fill_SCRC_Bank(SCRC_p, &kSCRC, j);
	   TClonesArray &tSCRCbank = *SCRCStore;
	   new(tSCRCbank[nSCRC_Store++]) TSCRCClass(SCRC_p);
	 }
       }
     ...
     //
       ...
       DC0    kDC0(0);
       TDC0Class   *DC0_p = new TDC0Class();
       if(kDC0.get_nrows() > 0){
	 for(int j = 0; j < kDC0.get_nrows(); j++){
	   Fill_DC0_Bank(DC0_p, &kDC0, j);
	   TClonesArray &tDC0bank = *DC0Store;
	   new(tDC0bank[nDC0_Store++]) TDC0Class(DC0_p);
	 }
       }
...
void Initialize_Branches(TTree *tree,TTree *scaler_tree){
  ...
  SCRCBranch = tree->Branch("SCRC",&fgSCRCStore,bufsize,split);
  //tree->SetBranchStatus("SCPB",1);
  
  DCPBBranch = tree->Branch("DCPB",&fgDCPBStore,bufsize,split);

  DC0Branch = tree->Branch("DC0",&fgDC0Store,bufsize,split);
...
void Init_Clones()
{
  ...
  if(!fgSCRCStore) fgSCRCStore = new TClonesArray("TSCRCClass",1);
  SCRCStore  = fgSCRCStore;
  
  
  if(!fgDCPBStore) fgDCPBStore = new TClonesArray("TDCPBClass",1);
  DCPBStore  = fgDCPBStore;

  if(!fgDC0Store) fgDC0Store = new TClonesArray("TDC0Class",1);
  DC0Store  = fgDC0Store;
...
void Clear_Clones()
{
  ...
  if(fgSCRCStore) SCRCStore->Clear();
  if(fgDCPBStore) DCPBStore->Clear();
  if(fgDC0Store) DC0Store->Clear();
