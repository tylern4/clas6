...
void seb_branches(TTree* tree) {
  ...
  /*
    Message in a bottle of scotch.

    Mai wanted to put a vagina joke here, but vagina jokes aren't
    funny, period.

    So I was going to do a dick joke instead, but they're all the
    same, just vary in length.
  */
  ...
  tree->Branch("dc_xec",&dcpb_nt_.dc_xec,"dc_xec[dc_part]/F");
  tree->Branch("dc_yec",&dcpb_nt_.dc_yec,"dc_yec[dc_part]/F");
  tree->Branch("dc_zec",&dcpb_nt_.dc_zec,"dc_zec[dc_part]/F");
  tree->Branch("dc_thcc",&dcpb_nt_.dc_thcc,"dc_thcc[dc_part]/F");
