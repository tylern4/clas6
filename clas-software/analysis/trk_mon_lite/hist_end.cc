...
int hist_end(void)
{
	// If autofinding local angles and only producing local angle histos,
	// then we are working only in memory so we don't have a file to flush
	// to.

	if(LOCAL_ANGLE_HISTOS_ONLY && AUTOFIND_LOCAL_ANGLE_CUTS)return 0;
...
void book_histos( int runno){
  ...
  for(i=2;i<10;i++){ 
    ...
    hbook1(3+offset,title,300,-15,15,0);
