...
c     RCS information: 
      ...
      DATA CRCSID/   
     1     '$Id: fill_seb_ntn.F,v 1.4 2002/03/25 22:12:39 stepanya Exp $'   
     2     /   
...
c     -----------------------------
      ...
      INTEGER ITRPB,ITGBI,IFBPM,IHLS,IRTSL
...
c     
      If(YES_PHOTON) then
         ind=0
         if(.not.flag_RTSL) ngamma=0
  	 ind =MLINK(jw,'RTSL',0)
         if(ind.ne.0) then
            ngamma=0
            flag_RTSL=.true.
            do i=1,64
               ngamma=ngamma+jW(ind+i)/1000
            enddo
            ngamma=ngamma/1000
         endif
      Endif
      ...
      hlsc=iw(ind+2)
      helicity=ihlst
      call uconvert(iw(ind+3),intt)
c      write(*,*) 'From fortran is ', iw(ind+3),'   INTT = ',intt
c      intt=0
c      do i=1,32
c         intt=intt+2**(jbit(iw(ind+3),i))
c      enddo
c      intt=iw(ind+3)
      ...
      l3bit=iw(ind+5)
c+
c      call mvbits (level1_trg_bit,0,6,l1bit,0)
c++ looping over  8 L1 bits (0-7) 
      inl1b=16
      do jj=1,16
       bb= btest(level1_trg_bit,jj-1)
       l1bit_v(jj)=0
       if (bb) l1bit_v(jj)=1
c       print *,'ff=',l1bit_v(jj)
      enddo 
c       print *,level1_trg_bit
c-
      ...
      IF(ind.NE.0) THEN
         ...
         if(ntrip.gt.0) then
            do i=1,ntrip
               if(evntid.ge.trip_min(i).AND.evntid.le.trip_max(i))
     &            evnttrip=trip_flag(i)
            enddo
         endif
         q_l=RW(ind+8)
         t_l=RW(ind+9)
...
c     
      ...
      If(ind.NE.0) Then
         ...
         do ix=1,nrow
            ...
            dc_part=i
            if(dc_stat(i).gt.0)tbtrk=tbtrk+1
            if(ind1.gt.0)then
...
ccc correlates sign of stat flag in EVNT bank with time base tracking
ccc       Stat should be negative if there is no TBT
      if(gpart.gt.0) then
         do i=1,gpart
            if(q(i).ne.0.AND.dc(i).gt.0.AND.dc(i).le.dc_part) then
               if(dc_stat(dc(i)).lt.0.AND.stat(i).gt.0) stat(i)=-stat(i)
            endif
         enddo
      endif
...
c     
      ...
      If(ind.NE.0) Then
         ...
         do ix=1,nd,ncol
            ...
            tag_stat(i) = iw(indx+1)-1000*tag_ptr(i)
      ...
      Do i=1,gpart
         if(dc(i).gt.0)then
            ...
            if(stat(i).gt.0.and.dc_stat(dc(i)).gt.0)then
               ntbttrk=ntbttrk+1
               sect=dc_sect(dc(i))
c                  if(i.eq.1.and.ec(i).gt.0.and.cc(i).gt.0.and.sc(i).gt.0.and.q(i).lt.0)then
c                     if(nphe(cc(i)).gt.20)then
c                        nelect(sect)=nelect(sect)+1
c                        if(evntclas.gt.0)then
c                           nelectp=nelectp+1
c                        elseif(evntclas.lt.0)then
c                           nelectn=nelectn+1
c                        endif
c                     endif
c                  endif
