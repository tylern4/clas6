...
/* -=======================================================-
RCS ID string
$Id: filter_tagr.c,v 1.4 2003/01/08 21:57:03 pasyuk Exp $
$Author: pasyuk $
$Revision: 1.4 $
$Date: 2003/01/08 21:57:03 $
* -=======================================================- */

#define USE(var) static void * use_##var = (void *) &var
static char crcsid[] = "$Id: filter_tagr.c,v 1.4 2003/01/08 21:57:03 pasyuk Exp $";
...
int ProcessEvent(int pid_group){
  ...
  if(TAGR && 
	...
	hf1(501, TAGR->tagr[i].tpho,1.0);
      ...
      hf1(63,TAGI->tagi[i].trf,1);
   }
    
    
    /* find "good" photon */
    
    tagr_t *tagr = get_photon_tagr(TAGR, TBID);
    if (tagr) {
