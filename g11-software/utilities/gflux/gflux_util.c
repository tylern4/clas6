/* gflux_util.c */
/* 	$Id: gflux_util.c,v 1.10 2004/08/30 19:32:15 weygand Exp $	 */

#ifndef lint
static char vcid[] = "$Id: gflux_util.c,v 1.10 2004/08/30 19:32:15 weygand Exp $";
#endif /* lint */
...
void bookHistos(){
  ...
  if(normRun){    
    ...
    hbook2(231, title, NUM_TDC, TDC_MIN, TDC_MAX, 80, -20., 20., 0);

    sprintf(title, "tpho - tacotime vs tpho, tagtnorm eq 1, %s", runMes);
    hbook2(232, title, NUM_TDC, TDC_MIN, TDC_MAX, 80, -20., 20., 0);
...
void fillHistos(){
  ...
  if(TAGR){
	...
	if(TACO && TAGR->tagr[i].t_id == TACO->taco[0].tid){
