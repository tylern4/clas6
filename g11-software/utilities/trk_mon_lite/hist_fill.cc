...
int hist_fill(void)
{
   ...
   // Stop now if we have enough statistics
	...
	for(int sec=1;sec<=6;sec++){
		...
		if(!TBLA)continue;
		tbla = TBLA->tbla;
      for(int i=0;i<TBLA->bank.nrow;i++, tbla++){
			...
			if(localangle>=LOCANGLE_CUT_LOW[reg]){
				if(localangle<=LOCANGLE_CUT_HIGH[reg]){
...
int EnoughEvents(void)
{
   ...
   if(TOTAL_NTUPLE_ROWS>=MAX_NTUPLE_ROWS){
      cout<<" Maximum number of Ntuple rows filled ("<<MAX_NTUPLE_ROWS<<")."<<endl<<endl;
      return 1;
   }
