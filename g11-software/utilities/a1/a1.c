...
_begin_doc
RCS ID string
$Id: a1.c,v 1.160 2004/05/07 03:24:19 pasyuk Exp $
$Author: pasyuk $
$Revision: 1.160 $
...
void PrintUsage(char *processName)
{
  ...
  fprintf(stderr, "\t[-T#]\tset the beam Type (0 = electron, 1 = gamma)\n");
  fprintf(stderr, "\t[-E#]\tset the beam Energy (MeV)\n");
  fprintf(stderr,"\t[-R#]\tinitalize with a different run number\n");
...
static void signalINT(int isig)
{
  ...
  if (count < 4){
    ...
    fparm_c(mess);
    sprintf(mess,"CLOSE BOSOUTPUT UNIT=8");
    fparm_c(mess);
    sprintf(mess,"CLOSE BOSOUTPUT UNIT=9");
    fparm_c(mess);
    sprintf(mess,"CLOSE BOSOUTPUT UNIT=10");
    fparm_c(mess);
    exit(0);
  }
}
...
/*    char *scalout = NULL; */
  ...
  for(i=1; i < argc; ++i){
	...
	fprintf(stderr,"Output file: %s\n",outfile);
	unlink(outfile);
	sprintf(out, "OPEN BOSOUTPUT UNIT=9 FILE=\"%s\" WRITE  STATUS=NEW RECL=3600", outfile);
	if (!fparm_c(out)) {
	  fprintf(stderr,"%s: Unable to open file \'%s\': %s\n\n",argv[0],out,strerror(errno));	 
	  exit(1);
	}
	...
	unlink(boutfile);
	sprintf(out, "OPEN BADBOS UNIT=8 FILE=\"%s\" WRITE STATUS=NEW RECL=3600", boutfile);
	if (!fparm_c(out)) {
	  fprintf(stderr,"%s: Unable to open file \'%s\': %s\n\n",argv[0],out,strerror(errno));	 
	  exit(1);
	}
  ...
  if (outfile==NULL){
    ...
    unlink(scalerfile);
    sprintf(out, "OPEN SCALERBOS UNIT=%d FILE=\"%s\" WRITE STATUS=NEW RECL=3600", ScalerOutUnitNo, scalerfile);
  ...
  for (i = 1; (max ? Nevents < max : 1) && ((i < argc) || server) ; ++i) {
	...
	if (!server) {
	  ...
	  fprintf(stderr,"Output file: %s\n",outfile);
	  unlink(outfile);
	  sprintf(out, "OPEN AUTOOUTPUT UNIT=%d FILE=\"%s\" WRITE STATUS=NEW RECL=3600", AutoOutputUnitNo, outfile);
	  if (!fparm_c(out)) {
	    fprintf(stderr,"%s: Unable to open file \'%s\': %s\n\n",argv[0],out,strerror(errno));	 
	    exit(1);
	  }
...
/*  		      if(NeutralEvent(nneut)) fprintf(stderr, "2n event %d\n",WriteEvent(select)); */
	...
	case DISIO_COMMAND:
	  ...
	  break;
	case DISIO_ERROR:
	  /* Assume broken pipe for time being, and remove ValidInput flag */
	  ValidInput = 0;
	  break;
	default:
	  break;
  ...
  if (boutfile) {
    putBOS(&bcs_, 8, "0");
    sprintf(mess,"CLOSE BADBOS UNIT=8");
    fparm_c(mess);
  }
  if (ScalerOutUnitNo) {
