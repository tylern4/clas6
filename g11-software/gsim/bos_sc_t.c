/*
 * $Id: bos_sc_t.c,v 1.3 2001/04/29 06:20:22 feuerbac Exp $
 *
...
static char cfile[] =  "$RCSfile: bos_sc_t.c,v $";
static char crevis[] = "$Revision: 1.3 $";
static char cstate[] = "$State: Exp $";
static char cdate[] =  "$Date: 2001/04/29 06:20:22 $";
...
/*	printf("Nhits: %d in sector %d\n",nhits,isec);	 */
  ...
  if(!gsimcontrol_.nodata[SC]){
    ...
    for(isec=1;isec<=6;isec++){
/* Look for SC bank in wcs area for preexisting SC hits*/
/*      SCjbank = (clasSC_t *)getGroup(&wcs_,"SC  ",isec);*/
...
/* Now store the digi in the SC bosbank. */
        ic0=0;   /* Counts old digi's that did not match with new ones*/
	if((SCjbank = (clasSC_t *)getGroup(&wcs_,"SC  ",isec))!=NULL){
          for(ic=0;ic<SCjbank->bank.nrow;ic++){
            ic0++;
            used[ic]=1;
	    for(idigi=0;idigi<ndigi;idigi++){
 ...
 */
	      ...
	      if(id==SCjbank->sc[ic].id){
	        ic0--;
	        used[ic]=-1;
/*	    printf("Number of new SC hits %d %d\n",ndigi,idigi);
	    printf("Old hits %d %d\n",SCjbank->bank.nrow,ic);
	    adclold=SCjbank->sc[ic].adcl;
	    adcrold=SCjbank->sc[ic].adcr;
	    tdclold=SCjbank->sc[ic].tdcl;
	    tdcrold=SCjbank->sc[ic].tdcr;
	    printf("Left ADC %d %d\n",kdigits[idigi][ADCL],adclold);
	    printf("Right ADC %d %d\n",kdigits[idigi][ADCR],adcrold);
	    printf("Left TDC %d %d\n",kdigits[idigi][TDCL],tdclold);
	    printf("Right TDC %d %d\n",kdigits[idigi][TDCR],tdcrold);
*/
	        kdigits[idigi][ADCL] = kdigits[idigi][ADCL]+SCjbank->sc[ic].adcl;
	        kdigits[idigi][ADCR] = kdigits[idigi][ADCR]+SCjbank->sc[ic].adcr;
	        if(kdigits[idigi][TDCL]>SCjbank->sc[ic].tdcl)kdigits[idigi][TDCL]=SCjbank->sc[ic].tdcl;
	        if(kdigits[idigi][TDCR]>SCjbank->sc[ic].tdcr)kdigits[idigi][TDCR]=SCjbank->sc[ic].tdcr;
	      }
	    }
	  }
	}

	if((SCbank = (clasSC_t *)makeBank(&bcs_,"SC  ",isec,5,num_to_bos+ic0))==NULL){
	  fprintf(stderr," ERROR-- Could not create new SC bank on output.\n");
	}else{
/*	    printf("Total SC entries %d %d\n",num_to_bos,ic0);
...
/*	    printf("SC banks field %d\n",ii);
            check_sc_bank_();
*/
	  if((SCjbank = (clasSC_t *)getGroup(&wcs_,"SC  ",isec))!=NULL){
            for(ic=0;ic<SCjbank->bank.nrow;ic++){
              if(used[ic]>0){
	        SCbank->sc[ii].id = SCjbank->sc[ic].id;
	        SCbank->sc[ii].tdcl = SCjbank->sc[ic].tdcl;
	        SCbank->sc[ii].adcl = SCjbank->sc[ic].adcl;
	        SCbank->sc[ii].tdcr = SCjbank->sc[ic].tdcr;
	        SCbank->sc[ii].adcr = SCjbank->sc[ic].adcr;
	        ii++;
	      }
            }
          }
        ...
        if((SCjbank = (clasSC_t *)getGroup(&wcs_,"SC  ",isec))!=NULL){
	  if((SCbank = (clasSC_t *)makeBank(&bcs_,"SC  ",isec,5,SCjbank->bank.nrow))==NULL){
	    fprintf(stderr," ERROR-- Could not create new SC bank on output.\n");
	  }else{
	    ii = 0;  /* Count the banks, not the digi's */
            for(ic=0;ic<SCjbank->bank.nrow;ic++){
