/*
 * $Id: geom_sc.c,v 1.6 2001/06/27 14:09:56 holtrop Exp $
 *
...
static char crevis[] = "$Revision: 1.6 $";
static char cstate[] = "$State: Exp $";
static char cdate[] =  "$Date: 2001/06/27 14:09:56 $";
...
void geom_sc_init(int isec,SC_geom_t *Geom){
 ...
 /*  Rm = (Xl+Xh)/2. ;  Len = |(Xl-Xh)| */    
  ...
  /* Seperate the ganged scintillators into seperate strips. */
  ...
  for(i= FIRST_SPLIT_PADDLE;i<Geom->SCGG->bank.nrow;i++){
    ...
    if(kk>= NUM_SCIN || panel_idx<0){
      fprintf(stderr,"ABORT, divided strips are more than total strips or panel error. \n");
      exit;
    }
