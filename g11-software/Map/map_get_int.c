...
/*         Purpose     : get array of int values from the map.  if no  */
     ...
     
{
  ...
  *vtime = -2;
  ...
  if ( strlen(subsystemname) > MAP_NAMELENGTH-1 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_int: Subsystem name (%s) longer than %d characters.",
		subsystemname,
		MAP_NAMELENGTH-1);
  ...
  if ( strlen(itemname) > MAP_NAMELENGTH-1 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_int: Item name (%s) longer than %d characters.",
		itemname,
		MAP_NAMELENGTH-1);
      return MAP_USER_ERROR_ARGUMENT;
    }
  ...
  if ( ctry == 1 )
  ...
  else
    {
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_int: Retrying for time %d, subsystem (%s), item (%s).\n",
	       atime,
	       subsystemname,
	       itemname);
  ...
  if ( fd < 0 )
    {
      map_syserror(fd,
		   "map_get_int: can't open TheMap (%s).",
		   filename);
      return fd;
    }
  ...
  if ( length != item.length )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_int: Array length (%d) incorrect for subsystem (%s),\n item (%s).  Should be %d.\n",
		length,
		subsystemname,
		itemname,
		item.length);
  ...
  if ( item.type != 0 )
    {
      map_error(MAP_USER_ERROR_ARGUMENT,
		"map_get_int: Array type (%d) for subsystem (%s), item (%s) \n requires call to different map_get_* routine.",
		item.type, 
		subsystemname,
		itemname);
      map_close_map(fd);
  ...
  if ( (item.list == NULL_LOC) || (item.list < 0 && item.table_used == 0 ))
    {
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_int: No values for subsystem (%s), item (%s).\n",
	       subsystemname,
	       itemname);
      *vtime = -1;
      map_close_map(fd);
  ...
  if ( succ_loc == NULL_LOC )
      ...
      if ( ctry == 1 )
	{
      ...
      map_warn(MAP_USER_WARN_NOFIND,
	       "map_get_int: No values at time %d for subsystem (%s), item (%s).\n",
	       atime,
	       subsystemname,
	       itemname);
      *vtime = -1;
  ...
  *vtime = succ_time;
  ...
  status = map_read(array,length*sizeof(int),fd,succ_loc+ARRAY_VALUES_OFFSET,swapintarray);
  if ( status != MAP_OK ) 
    {
      map_close_map(fd);
      return status;
