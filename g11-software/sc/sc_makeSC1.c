...
/* -=======================================================- *
$Id: sc_makeSC1.c,v 1.27 2003/02/23 03:07:33 langhei Exp $
$Author: langhei $
$Revision: 1.27 $
$Date: 2003/02/23 03:07:33 $
* -=======================================================- */
...
clasSC1_t *make_SC1_bank(int sector){
  ...
  clasSC1_t *SC1=NULL;
  clasSCG_t *SCG=NULL;
  sc1_t sc1arr[SC_NPADDLES_SEC];
  int  tdcl_swapped[SC_NPADDLES];
  int  tdcr_swapped[SC_NPADDLES];
  int nswap = 0;
  int cal_index;
...
/* The following code corrects for miscabling at TOF patch panel for left TDCs during e1e run period */
	...
	cal_index = sc_index(sector, SC->sc[scBank_index].id);
...
/*check paddle id, map status and tdc values - make sure they are valid*/
	...
	cal_index = sc_index(sector,SC->sc[scBank_index].id);
	sc1arr[nSC1].id = SC->sc[scBank_index].id;
...
int valid_SC_hit(clasSC_t *SC, int sector, int index){
  int map_index;
  if (SC->sc[index].id > 0 && SC->sc[index].id <= SC_NPADDLES_SEC){
    /* check the hw status of the left side */
    switch(sc_status.left[sc_index(sector, SC->sc[index].id)]){
    ...
    switch(sc_status.right[sc_index(sector, SC->sc[index].id)]){
    case SC_STATUS_NO_ADC:
      SC->sc[index].adcr = 0;
      break;
    case SC_STATUS_NO_TDC:
