...
/*_begin_doc
 *  RCS ID string
 *  $Id: vertex_mtv.cc,v 1.3 1999/02/19 19:52:52 feuerbac Exp $*/
...
int mtv(int *tracks, int ntrk,vector3_t *vert,float Cmat[9],float *chi2,int *iter){ 
  ...
  try{
	...
	return(13);
      ...
      Ct[track](LAMBDA,LAMBDA)=my_tber.get_c22(this_track);
      Ct[track](PHI,PHI)      =my_tber.get_c33(this_track);
      Ct[track](D0,D0)        =my_tber.get_c44(this_track);
      Ct[track](Z0,Z0)        =my_tber.get_c55(this_track);

      Ct[track](PHI,LAMBDA)=Ct[track](LAMBDA,PHI)=my_tber.get_c23(this_track);
      Ct[track](D0,LAMBDA) =Ct[track](LAMBDA,D0) =my_tber.get_c24(this_track);
      Ct[track](Z0,LAMBDA) =Ct[track](LAMBDA,Z0) =my_tber.get_c25(this_track);
      Ct[track](D0,PHI)    =Ct[track](PHI,D0)    =my_tber.get_c34(this_track);
      Ct[track](Z0,PHI)    =Ct[track](PHI,Z0)    =my_tber.get_c35(this_track);
      Ct[track](Z0,D0)     =Ct[track](D0,Z0)     =my_tber.get_c45(this_track);

      //get track info
      lambda=my_tber.get_lambda(this_track);
      phi=my_tber.get_phi(this_track);
      d0 = my_tber.get_d0(this_track);
      z0=my_tber.get_z0(this_track);
      layinfo2=my_tber.get_layinfo2(this_track);
