...
RECSIS_SHARED_LIBS += -ltcl$(TCL_VERSION) -lgfortran -lnsl -ldl -lstdc++ 
#RECSIS_SHARED_LIBS += -lnsl -ldl

# /lib/libdl.so.1
RECSIS_MD_LIBS += fputil$(ADD_DEBUG) bos$(ADD_DEBUG) fpack$(ADD_DEBUG)
RECSIS_MD_LIBS += patches$(ADD_DEBUG) 
...
#GSIM_INT_LIBS0  += recutl  clasutil 
#GSIM_INT_LIBS0  += c_bos_io bosio online_dummy
...
GSIM_INT_LIBS  += mathlib kernlib

ifndef MAP
  GSIM_INT_SHARED_LIBS += -lmysqlclient -ltcl$(TCL_VERSION)
  GSIM_INT_MD_LIBS  += z 
endif
...
GSIM_INT_SHARED_LIBS += -lnsl -lcrypt -ldl /usr/lib/liblapack.so.3 /usr/lib/libblas.so.3

GSIM_BAT_MD_LIBS = nsl patches$(ADD_DEBUG)
GSIM_BAT_SHARED_LIBS += -lX11 /usr/lib/liblapack.so.3 /usr/lib/libblas.so.3
