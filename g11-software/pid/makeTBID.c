...
/* -=======================================================- *
$Id: makeTBID.c,v 1.35 2004/05/25 21:08:50 devita Exp $
$Author: devita $
$Revision: 1.35 $
$Date: 2004/05/25 21:08:50 $
* -=======================================================- */
...
/*   fprintf(stderr,"TBID Got all charged tracks\n"); */
  ...
  if(nTBID){
	...
	/* if it's neutral and the first tbid is charged*/ 
	  ...
	  if (ECHB = getBank(&bcs_, "ECHB")){
	    if(TBID->bid[i].ec.time > TBID->bid[0].sc.vtime){
	      /* target position must be taken into account */
	      neutral_path_length = sqrt(pow(ECHB->echb[TBID->bid[i].ec.id -1].x_hit - beam_position.x, 2)\
					 + pow(ECHB->echb[TBID->bid[i].ec.id -1].y_hit - beam_position.y, 2)\
					 + pow(ECHB->echb[TBID->bid[i].ec.id -1].z_hit - target_z(), 2));
