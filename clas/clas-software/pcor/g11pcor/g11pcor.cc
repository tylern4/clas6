...
// Function prototypes:
int GetSector(const float __p3[3]);

int GetBin(double __theta,double __phi,const double __phiBins_min[12],
	   const double __phiBins_max[12],const double __thetaBins_min[15],
	   const double __thetaBins_max[15]);			

void ReadBinsFile(double __phiBins_min[12],double __phiBins_max[12],
		  double __thetaBins_min[15],double __thetaBins_max[15]);

double GetLambdaTrack(const float __p3[3]);

double GetPhiTrack(const float __p3[3]);

void SetP3FromTrackingParameters(double __p,double __lambda,double __phi,
				 int __sector,float __p3[3]);

double GetSectorPhi(double __phi);

void Init_g11pcor(double __p_vals[2][6][180][3],int __num_pars_p[2][6][180],
...
void g11TaggerCor_(float *__eg,float *__e_beam,int *__eid,int *__runNumber,
		   float *__egCor){
  *__egCor = g11TaggerCor(*__eg,*__e_beam,*__eid,*__runNumber);
}

void g11MomCor_(float __p3[3],int *__q,float __p3cor[3]){
  g11MomCor(__p3,*__q,__p3cor);
}

void g11FiducialCut_(float __p3[3],int *__q,int *__cut){
  bool is_good = g11FiducialCut(__p3,*__q);
  if(is_good) *__cut = 1;
  else *__cut = 0;
}
//_____________________________________________________________________________

/// Tagger correction for the g11 run period
...
float g11TaggerCor(float __eg,float __e_beam,int __eid,int __runNumber){
  ...
  if(first_call){ // only do this on the 1st call to the function
    ...
    char file[200];
    sprintf(file,"%s/utilities/g11pcor/tagger_cor.dat",getenv("CLAS_PACK"));
    std::ifstream inFile_egcor(file);
    // check to make sure the file opened properly
    if(!inFile_egcor.is_open()){
      printf("Error!!!! <g11TaggerCor> Can NOT open tagger_cor.dat. This ");
      printf("file should be located in CLAS_PACK/utilites/g11pcor. Check ");
    ...
    if(count != 767){
      printf("Error!!!! <g11TaggerCor> Read incorrect number of E-paddles ");
      printf("[%d instead of 767]. \n",count);
      return -666.;
    }
    // read in beam offsets
    sprintf(file,"%s/utilities/g11pcor/beam_offset.dat",getenv("CLAS_PACK"));
  ...
  if(__eid < 1 || __eid > 767){
    printf("Warning <g11TaggerCor> E-paddle out of range [id = %d].",__eid);
...
void g11MomCor(float __p3[3],int __q,float __p3cor[3]){
  ...
  static double p_vals[2][6][180][3];
  ...
  if(firstCall){ // on first call, do the following...
    ...
    ReadBinsFile(phiBins_min,phiBins_max,thetaBins_min,thetaBins_max);
    // read in momcor parameters
    Init_g11pcor(p_vals,num_pars_p,phi_vals,num_pars_phi,lambda_vals,
		 num_pars_lambda);
    firstCall = false;
  }
  ...
  double lam = GetLambdaTrack(__p3),lam_cor;
  double phi = GetPhiTrack(__p3),phi_cor;
  double par[4];
  int sector,q_index;
  sector = GetSector(__p3);

  if(__q < 0) q_index = 1;
  else q_index = 0;

  bin = GetBin(theta_lab,phi_lab,phiBins_min,phiBins_max,thetaBins_min,
	       thetaBins_max);
  ...
  double p_scale = 1.;
  if(p > 2.) p_scale = 1.3;
  for(int i = 0; i < 3; i++) par[i] = p_vals[q_index][sector-1][bin][i];
  ...
  SetP3FromTrackingParameters(p_cor,lam_cor,phi_cor,sector,__p3cor);  
}
//_____________________________________________________________________________
...
bool g11FiducialCut(float __p3[3],int __q){
  ...
  phi = GetSectorPhi(phi)*180./3.14159; // sector phi in degrees
  if(__q > 0){
    if(theta <= 10.){
...
void ReadBinsFile(double __phiBins_min[12],double __phiBins_max[12],
  ...
  char file[200];
  sprintf(file,"%s/utilities/g11pcor/binning.dat",getenv("CLAS_PACK"));
  std::ifstream binsFile(file);
  int num_bins;

  if(!binsFile.is_open()){ // couldn't open the file
    printf("Error!!! <ReadBinsFile> Could NOT open binning.dat. ");
    printf("This function is called by g11MomCor and should be located in ");
    printf("CLAS_PACK/utilities/g11pcor, make sure your CLAS_PACK ");
...
void Init_g11pcor(double __p_vals[2][6][180][3],int __num_pars_p[2][6][180],
  ...
  file_base += "/utilities/g11pcor/pars_";
  char fileName[200];
  ...
  for(int s = 1; s <= 6; s++){ // loop over sectors
	...
	printf("Error!!!! <Init_g11pcor> Could NOT open %s. ",fileName);
	printf("This function is called by g11MomCor to read in the ");
