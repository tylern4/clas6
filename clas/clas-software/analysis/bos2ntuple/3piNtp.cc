...
int main(int argc,char **argv){
  ...
  for (int i = 1; i < argc; ++i) {
	...
	if(DISIO_DATA == ret && event.status() ){
	  ...
	  // 
	  if(event.N(PiMinus)==1 && event.N(PiPlus)==2
	     && event.N(-1)==1 && event.N(1)==2){
	    
	    nSelectedEvents++;
	    
	    fourVec pim, pip1, pip2, beam, miss4v;
	    ...
	    miss4v = beam + target - pim -pip1 -pip2;
	    ...
	    //
	    values[n_vectors++] = -(~(beam -(pip1+pip2+pim))); 
	    if(firstEvent){
	      sprintf(label,"t");
	      vnames->Add(label);
	      cout<<"t:\t\t  t(gamma - X) \n";
	    }
	    values[n_vectors++] = ~(pip1+pip2+pim); 
	    if(firstEvent){
	      sprintf(label,"mass3pi");
	      vnames->Add(label);
	    ...
	    if(firstEvent){
	      sprintf(label,"npip1");
	      vnames->Add(label);
	      cout<<"npip1:\t\t  Mass(pip1 n)\n";
	    }
	    values[n_vectors++] = ~(miss4v + pip2); 
	    if(firstEvent){
	      sprintf(label,"npip2");
	      vnames->Add(label);
	      cout<<"npip2:\t\t  Mass(pip2 n)\n";
	    }
	    values[n_vectors++] = ~(miss4v + pim); 
	    if(firstEvent){
	      sprintf(label,"npim");
	      vnames->Add(label);
	      cout<<"npim:\t\t  Mass(pim n)\n";
	    }
    ...
    }// end of processing a bos files
  }

  cerr << "Finished Processing\n";
