...
typedef struct {
	...
	vector3_t vert;     /*  vector3_t vert; Vertex position {x,y,z}  */
	vector4_t p;        /*  vector4_t p; Energy */
	int       q;        /*  Charge */
	...
	float     mass;     /*  Mass from time-of-flight */
        int       mass_ref; /*  0: mass calc from SC & TAG, 1: SC & ST; */ 
                            /*  -1 neutral or no SC;2:from PART */
	float     betam;    /*  Beta from time-of-flight */
...
/* ------------------------ hlb -----------------*/
typedef struct {
	uint32 b1;    /*  C22A Current */
	uint32 b2;    /*  C22A X-position */
	uint32 b3;    /*  C22A Y-position */
	uint32 b4;    /*  not used */
	uint32 b5;    /*  C24A Current */
	uint32 b6;    /*  C24A X-position */
	uint32 b7;    /*  C24A Y-position */
	uint32 b8;    /*  not used */
	uint32 b9;    /*  CH01 Current */
	uint32 b10;    /*  CH01 X-position */
	uint32 b11;    /*  CH01 Y-position */
	uint32 b12;    /*  not used */
	uint32 b13;    /*  not used */
	uint32 b14;    /*  not used */
	uint32 b15;    /*  not used */
	uint32 b16;    /*  HLS/B banks accumulating counter */
} hlb_t;
...
typedef struct {
                ...
                float t_next;    /*  Time to next closest hit */
...
typedef struct {
        float etot;    /*  Reconstructed total energy  */
        float ecen;    /*  Energy in the central crystal */
        float time;    /*  Time relative to the evnt start time */
        float t_next;    /*  Time to next closest hit */
        float x;    /*  x coordinate of hit  */
        float y;    /*  y coordinate of hit */
        float z;    /*  z coordinate of hit (front face of IC) */
        float m2_hit;    /*  second moment of _hit pattern */
        float m3_hit;    /*  third moment of  _hit pattern  */
        int status;    /*  Ncrystals+10000*Hit_ID in ICHB  */
} icpb_t;
...
typedef struct {
	...
	int status;    /*  Status word */
	float ein;     /* Reconstructed energy in inner part */ 
} lcpb_t;
...
/* ------------------------ lsrt -----------------*/
typedef struct {
	uint16 id;     /*  the address of the laser reference */
	uint16 tdc;    /*  tdc information (channels) */
} lsrt_t;
...
/* ------------------------ ms1 -----------------*/
typedef struct {
        uint16 id;    /*  the address of the struck microstrip/panel (4 panels) */
        uint16 adc;    /*  adc information (channels) */
} ms1_t;
...
/* ------------------------ rc26 -----------------*/
typedef struct {
	uint32 trigger_pattern;    /*  level1 trigger pattern from trigger supervisor */
	uint32 gated_scaler;    /*  gated scaler */
	uint32 ungated_scaler;    /*  ungated scaler */
} rc26_t;
...
/* ------------------------ ref -----------------*/
typedef struct {
	uint16 id;     /* roc id (high byte) & slot number (low byte) */
	uint16 tdc;    /*  tdc information (channels) */
} ref_t;
...
/* ------------------------ sct -----------------*/
typedef struct {
	uint16 id;     /*  the address of the TDC of the hit detector element */
	uint16 tdc;    /*  tdc information (channels) */
} sct_t;
...
/* ------------------------ shhb -----------------*/
typedef struct {
	int hid;    /*  channel number (quadrant*100 + channel) */
	float hx;    /*  lab coordinate X, */
	float hy;    /*  lab coordinate Y, */
	float hz;    /*  lab coordinate Z, */
	float hnphe;    /*  number of photoelectrons */
	float htime;    /*  t */
	int hstatus;    /* channel hit status      */
} shhb_t;
...
/* ------------------------ shpb -----------------*/
typedef struct {
	int id;    /*  channel number (quadrant*100 + channel) */
	float x;    /*  lab coordinate X, */
	float y;    /*  lab coordinate Y, */
	float z;    /*  lab coordinate Z, */
	float nphe;    /*  number of photoelectrons */
	float time;    /*  t */
	int status;    /* pixel hit status      */
} shpb_t;
...
/* ------------------------ stre -----------------*/
typedef struct {
        int id;        /*  sector*100 + paddle*10 + (leg=1, nose=2) */
        int trk_no;    /*  Pointer to track in TDPL */
        float st_time;    /*  flight time from ST (ns) */
        float st_l;    /*  flight path from ST (cm) */
        float st_pos;    /*  position within the start counter */
        int status;    /*  Status word */
...
/* ------------------------ trgs -----------------*/
typedef struct {
	uint32 clock_ug;    /*  Clock ungated */
	uint32 fcup_ug;    /*  FCUP ungated */
	uint32 microsec;    /*  Microsecond clock (will overflow during normal run) */
	uint32 random_ug;    /*  Random pulser ungated */
	...
	uint32 st;    /*  ST */
	uint32 hel_sync;    /*  Helicity sync signal */
	uint32 clock_ug_2;    /*  Duplicate of channel 1 */
	uint32 fcup_ug_2;    /*  Duplicate of channel 2 */
	uint32 clock_g1;    /*  Clock with run gatei */
	uint32 fcup_g1;    /*  FCUP with Run Gate */
	uint32 notused19;    /*  Currently not used */
	uint32 random_g1;    /*  Random pulser run gated */
	...
	uint32 random_g1_2;    /*  duplicate of channel 20 */
	uint32 clock_g1_2;    /*  duplicate of channel 17 */
	uint32 fcup_g1_2;    /*  duplicate of channel 18 */
	uint32 clock_g2;    /*  CLOCK live gated */
	uint32 fcup_g2;    /*  FCUP live gated */
	uint32 trig_or_g2;    /*  Trigger OR of 1-12 live gated */
	uint32 random_g2;    /*  Random pulser live gated */
	...
	uint32 mor_lg;    /*  MASTER_OR live gated */
	...
	uint32 random_g2_2;    /*  duplicate of channel 36 */
	uint32 clock_g2_2;    /*  duplicate of channel 33 */
	uint32 fcup_g2_2;    /*  duplicate of channel 34 */
	...
	uint32 l1accept;    /*  Level 1 accept */
	uint32 notused63;    /*  Currently not used */
	uint32 notused64;    /*  Currently not used */
	uint32 l2fail;    /*  Level2 fail */
	uint32 l2pass;    /*  Level2 pass */
	uint32 l2start;    /*  Level2 start */
	uint32 l2clear;    /*  Level2 clear */
	uint32 l2accept;    /*  Level2 accept */
	uint32 l3accept;    /*  Level3 accept */
	uint32 notused71;    /*  Currently not used */
	uint32 notused72;    /*  Currently not used */
	uint32 l2sec1_g;    /*  Level2 sec1 gated */
	uint32 l2sec2_g;    /*  Level2 sec2 gated */
	uint32 l2sec3_g;    /*  Level2 sec3 gated */
	uint32 l2sec4_g;    /*  Level2 sec4 gated */
	uint32 l2sec5_g;    /*  Level2 sec5 gated */
	uint32 l2sec6_g;    /*  Level2 sec6 gated */
	uint32 l2_or_g;    /*  OR level2 gated */
	uint32 l2_ok_g;    /*  Level 2 OK gated */
} trgs_t;
...
/* ------------------------ tlv1 -----------------*/
typedef struct {
	unsigned short id;    /*  id = Detector * 256 + channel */
	unsigned short time;    /*  time(ns)  */
} tlv1_t;
...
/* ------------------------ tpch -----------------*/
typedef struct {
	uint32 nevent;    /*  Event Number (starting with 1) */
	uint32 time;    /*  Time stamp */
	uint32 ntpc;    /*  Number of TPC rows */
} tpch_t;
