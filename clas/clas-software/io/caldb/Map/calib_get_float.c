/* 
 *  calib_get_float.c: get array of int values into the calib database
 *  Created        : 25-MAY-2000 by Riad Suleiman
 *  $Id: calib_get_float.c,v 1.4 2002/11/07 20:57:29 marki Exp $
 */
...
int calib_get_float( const char filename[], const char subsystemname[], 
		const char itemname[], int length, float farray[], int atime , 
		int *vtime )
{
  ...
  char intstr[100], value[100000]; /* should this be dynamically allocated? DL */
  int check, k;
 
	if(strstr(filename,"DC_DOCA")){
		k=1;
	}
  ...
  if(CALDB_VERBOSE)printf("calib_get_float PING = %i\n", mysql_ping(&mysql));
  ...
  if (mysql_real_query(&mysql, sql_cmd, len)) {
    status=1;
    printf("calib_get_float SQL_ERR: %s\n", mysql_error(&mysql));
    printf("calib_get_float SQL_ERR: query error %s\n", sql_cmd);
  }
