//$Id: stn_calib-g12.cc,v 1.1 2008/05/27 20:25:23 mukesh Exp $
using namespace std;
...
TH2F* hsctag[4], *htagsc[4], *hsttag[4], *hstsc[4], *htagst[4],*hsttagneg[4],*hsttagpos[4],*hstscneg[4],*hstscpos[4];  //Medit
TH2F* hcalib1, *hcalib2;
...
void book_histo(int runno) {
  ...
  */
  ...
  for (int i = 0; i< 4; i++) {
    ...
    hsctag[i] = new TH2F (hstname, hstname, SC_MAX, -0.5, SC_MAX-0.5, 1800, -30., 30.);
    hsctag[i]->SetOption("colz");

    sprintf (hstname, "tagsc_%s", cpart[i]);
    htagsc[i] = new TH2F (hstname, hstname, T_MAX, -0.5, T_MAX-0.5, 1500, -15., 15.);
    htagsc[i]->SetOption("colz");
    
    sprintf (hstname, "tagst_%s", cpart[i]);
    htagst[i] = new TH2F (hstname, hstname, T_MAX, -0.5, T_MAX-0.5, 1500, -15., 15.);
    htagst[i]->SetOption("colz");
    ...
    hsttag[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 1000, -10., 10.);
    hsttag[i]->SetOption("colz");
    
    hcalib1 = new TH2F ("calib1", "calib1", 24, -0.5, 23.5, 1000, -10., 10.);
    hcalib1->SetOption("colz");

 	hcalib2 = new TH2F ("calib2", "calib2", 24, -0.5, 23.5, 1000, -10., 10.);
    hcalib2->SetOption("colz");
    
    sprintf (hstname, "sttagneg_%s",cpart[i]);
    hsttagneg[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 1000, -10., 10.);
    hsttagneg[i]->SetOption("colz");

    sprintf (hstname, "sttagpos_%s",cpart[i]);
    hsttagpos[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 1000, -10., 10.);
    hsttagpos[i]->SetOption("colz");

    sprintf (hstname, "stsc_%s", cpart[i]);
    hstsc[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 799, -20., 20.);
    hstsc[i]->SetOption("colz");

	sprintf (hstname, "stscneg_%s",cpart[i]);
    hstscneg[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 1000, -10., 10.);
    hstscneg[i]->SetOption("colz");

    sprintf (hstname, "stscpos_%s",cpart[i]);
    hstscpos[i] = new TH2F (hstname, hstname, 24, -0.5, 23.5, 1000, -10., 10.);
    hstscpos[i]->SetOption("colz");


    sprintf (hstname, "sttime_%s",  cpart[i]);
    hsttime[i]  = new TH1F(hstname, hstname, 800, -10., 10.);
...
int analyze_event(istream* rawStream) {
  ...
  for (vector<MyPart>::iterator mpi = mypart.begin(); /// loop particles
	...
	if (mpi->charge < 0 && pid==0) 
	    hsttagneg[pid]->Fill(ii,dt);
	...
	//MEDIT
	htagst[pid]->Fill(tagr[starttime.tagrIndex].t_id-1.,dt);

	/// ToF entry
	if (mpi->sc.index >= 0) {
	  double dtsc = ht->GetTime() - t_to_st - mpi->sc.t0;
	  hstsc[pid]          ->Fill(ii, dtsc);
	  double yahoo = mpi->st.t0 - mpi->sc.t0;
	  hcalib1->Fill(ii,yahoo);
	
	if (mpi->charge < 0) {
	  double yahoom = mpi->st.t0 - mpi->sc.t0;
	  hcalib2->Fill(ii,yahoom);
	}
	if (mpi->charge < 0) {
	    hstscneg[pid]->Fill(ii,dtsc);
	    //double yahoom = mpi->st.t0 - mpi->sc.t0;
	    //hcalib2->Fill(ii,yahoom);
	}   
	//else
	  //  hstscpos[pid]->Fill(ii,dtsc); 
