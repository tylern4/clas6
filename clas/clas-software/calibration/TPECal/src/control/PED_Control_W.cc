...
PED_Control_W :: PED_Control_W(QWidget *mom, QSplashScreen* spl, QApplication *ecg, 
...
void PED_Control_W::show_peds()
{
	...
	TPECal_calib->ped_graphs(1);  ///< builds  TGraphErrors
	
	// determining max of graphs
	float max = 0;
	for(int ss=0; ss<30; ss++)
		if(fabs(TPECal_calib->ped_value[ss]) > max) max = fabs(TPECal_calib->ped_value[ss]);
	max = max*1.2;
	
	float min = 100;
	for(int ss=0; ss<30; ss++)
		if(fabs(TPECal_calib->ped_value[ss]) < min) min = fabs(TPECal_calib->ped_value[ss]);
	min = min*0.8;
	
	spad->cd(1);
	hr = gPad->DrawFrame(0, min, 30, max);
	hr->SetXTitle(Form("TPE Pedestals "));
	hr->SetTitleOffset(1.2);
	hr->SetTitleSize(.04);
	hr->SetLabelSize(.001, "XY");
	//TPECal_calib->PMeanO->Draw("LP");
	TPECal_calib->PMean->Draw("LP");
	...
	me.DrawLatex(.15, .90, Form("Run %d", TPECal_calib->runno));
...
//	me.DrawLatex(.75, .95, "#diamond DB Values ");
	...
	TPECal_calib->ped_graphs(0); ///< deletes  TGraphErrors
	delete hr;
	delete spad;
