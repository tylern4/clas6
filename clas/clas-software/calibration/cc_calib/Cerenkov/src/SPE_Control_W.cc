...
SPE_Control_W :: SPE_Control_W(QWidget *mom, QSplashScreen* spl, QApplication *ccg, cc_opts *op, cc_calib *ccc, TCanvas *Sp, QLabel* La) : QWidget(mom)
{
	splash   = spl;
	cc_gui   = ccg;
	ccOpt    = op;
	...
	MButtonW->setGeometry(2, 2, 140, 300);
	...
	connect ( MainButtons[4], SIGNAL( clicked() ), mom,   SLOT( write_spe_db() ) );
...
void SPE_Control_W::show_spes()
{
	...
	Line->SetLineColor(2);
	CC_calib->spe_graphs(1);  ///< builds  TGraphErrors

	// determining min, max of graphs
	float max = 0;
	for(int s=0; s<6; s++)
		for(int ss=0; ss<36; ss++)
			if(fabs(CC_calib->spe_value[s][ss]) > max) max = fabs(CC_calib->spe_value[s][ss]);
	max = max*1.2;
	float min = 100;
	for(int s=0; s<6; s++)
		for(int ss=0; ss<36; ss++)
			if(fabs(CC_calib->spe_value[s][ss]) < min) min = fabs(CC_calib->spe_value[s][ss]);
	min = min*0.8;
	...
	for(int s=0; s<6; s++)
	{
		...
		CC_calib->MeanO[s]->Draw("P");
		CC_calib->Meanu[s]->Draw("P");
		CC_calib->MeanD[s]->Draw("P");
		CC_calib->MeanM[s]->Draw("P");
		Line->Draw("same");
	}
	...
	me.DrawLatex(.03, .94, "Single Photo-Electron Peak ");
	me.DrawLatex(.17, .90, Form("Run %d", CC_calib->runno));
	...
	CC_calib->spe_graphs(0); ///< deletes  TGraphErrors
	delete Line;
	delete hr;
	delete spad;
