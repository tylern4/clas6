...
Float_t getEpsL(Float_t E_beam, Float_t W, Float_t Q2)
{
  ...
  // cout << E_beam<<" "<<W<<" "<<Q2<<" "<<nu<<" "<<theta_el<<" "
  // <<Q2/E_beam/(E_beam - nu)/2 <<" "<<eps_t<<" "<< eps_l << "\n";
  return eps_l;
}
...
Float_t pol2(Float_t x, Short_t wbin, Short_t i)
{
  ...
  if (i == 1)
    func = FIT_PARAM_SIGMA_L[2][wbin] * x * x + FIT_PARAM_SIGMA_L[1][wbin] * x +
           FIT_PARAM_SIGMA_L[0][wbin];
  if (i == 2)
    func = FIT_PARAM_SIGMA_C2F[2][wbin] * x * x +
           FIT_PARAM_SIGMA_C2F[1][wbin] * x + FIT_PARAM_SIGMA_C2F[0][wbin];
  if (i == 4)
    func = FIT_PARAM_SIGMA_CF[2][wbin] * x * x +
           FIT_PARAM_SIGMA_CF[1][wbin] * x + FIT_PARAM_SIGMA_CF[0][wbin];
  return func;
}

void get_xsect_14_18_lowq2_fit(
    ...
    Float_t &sigma_cf_final, Float_t &sigma_sf_final)
{
  ...
  Short_t s12left_wleft_bin =
      getsbin(Wleft_bin, s12gen, S12_ARR[11][Wleft_bin], S12_ARR[0][Wleft_bin]);
  Short_t s12right_wleft_bin = s12left_wleft_bin + 1;

  Short_t s12left_wright_bin = getsbin(
      Wright_bin, s12gen, S12_ARR[11][Wright_bin], S12_ARR[0][Wright_bin]);
  Short_t s12right_wright_bin = s12left_wright_bin + 1;

  Short_t s23left_wleft_bin =
      getsbin(Wleft_bin, s23gen, S23_ARR[11][Wleft_bin], S23_ARR[0][Wleft_bin]);
  Short_t s23right_wleft_bin = s23left_wleft_bin + 1;

  Short_t s23left_wright_bin = getsbin(
      Wright_bin, s23gen, S23_ARR[11][Wright_bin], S23_ARR[0][Wright_bin]);
  ...
  // then we are doing 4d-interpolation for Wleft_bin and Wright_bin and obtain
  ...
  for (Short_t i = 0; i < 6; i++)
  {
    ...
    interpol(4, Q2_bin, Wleft_bin, s12left_wleft_bin, s12right_wleft_bin,
  ...
  // sigma_t
  sigma_wright[0] = sigma_wright[0] * func_sigma_t(Q2gen, Wright_bin) /
                    func_sigma_t(0.65, Wright_bin);
  sigma_wleft[0] = sigma_wleft[0] * func_sigma_t(Q2gen, Wleft_bin) /
                   func_sigma_t(0.65, Wleft_bin);

  // sigma_l
  sigma_wright[1] =
      sigma_wright[1] * pol2(Q2gen, Wright_bin, 1) / pol2(0.65, Wright_bin, 1);
  sigma_wright[1] = sigma_wright[1] * getEpsL(2.445, W_ARR[Wright_bin], 0.65) /
                    getEpsL(2.445, W_ARR[Wright_bin], Q2gen);

  // sigma_c2f
  sigma_wright[2] = sigma_wright[2] * Func_q2_dep(Q2gen) / Func_q2_dep(0.65);
  // sigma_wright[2] = sigma_wright[2]*(pol2(Q2gen,Wright_bin,2)
  // -1.)/(pol2(0.65,Wright_bin,2) -1.);  sigma_wright[2] =
  // sigma_wright[2]*getEpsT(2.445,W_ARR[Wright_bin],0.65)/getEpsT(2.445,W_ARR[Wright_bin],Q2gen);
  // sigma_wright[2] = sigma_wright[2]*
  // func_sigma_t(Q2gen,Wright_bin)/SIGMA_T_INT_RIPANI[Wright_bin];

  // sigma_cf
  sigma_wright[4] = sigma_wright[4] * Func_q2_dep(Q2gen) / Func_q2_dep(0.65);
  // sigma_wright[4] = sigma_wright[4]*(pol2(Q2gen,Wright_bin,4)
  // -1.)/(pol2(0.65,Wright_bin,4) -1.);  sigma_wright[4] =
  // //sigma_wright[4]*sqrt(2*getEpsL(2.445,W_ARR[Wright_bin],0.65)*(getEpsT(2.445,W_ARR[Wright_bin],0.65)+1))/sqrt(2*getEpsL(2.445,W_ARR[Wright_bin],Q2gen)*(getEpsT(2.445,W_ARR[Wright_bin],Q2gen)+1));
  // sigma_wright[4] =
  // sigma_wright[4]*func_sigma_t(Q2gen,Wright_bin)/SIGMA_T_INT_RIPANI[Wright_bin];
  ...
  ////sigma_l
  sigma_wleft[1] =
      sigma_wleft[1] * pol2(Q2gen, Wleft_bin, 1) / pol2(0.65, Wleft_bin, 1);
  sigma_wleft[1] = sigma_wleft[1] * getEpsL(2.445, W_ARR[Wleft_bin], 0.65) /
                   getEpsL(2.445, W_ARR[Wleft_bin], Q2gen);
  // sigma_c2f
  sigma_wleft[2] = sigma_wleft[2] * Func_q2_dep(Q2gen) / Func_q2_dep(0.65);
  // sigma_wleft[2] = sigma_wleft[2]*(pol2(Q2gen,Wleft_bin,2)
  // -1.)/(pol2(0.65,Wleft_bin,2) -1.);  sigma_wleft[2] =
  // sigma_wleft[2]*getEpsT(2.445,W_ARR[Wleft_bin],0.65)/getEpsT(2.445,W_ARR[Wleft_bin],Q2gen);
  // sigma_wleft[2] = sigma_wleft[2]*
  // func_sigma_t(Q2gen,Wleft_bin)/SIGMA_T_INT_RIPANI[Wleft_bin];

  // sigma_cf
  sigma_wleft[4] = sigma_wleft[4] * Func_q2_dep(Q2gen) / Func_q2_dep(0.65);
  // sigma_wleft[4] = sigma_wleft[4]*(pol2(Q2gen,Wleft_bin,4)
  // -1.)/(pol2(0.65,Wleft_bin,4) -1.);  sigma_wleft[4] =
  // sigma_wleft[4]*sqrt(2*getEpsL(2.445,W_ARR[Wleft_bin],0.65)*(getEpsT(2.445,W_ARR[Wleft_bin],0.65)+1))/sqrt(2*getEpsL(2.445,W_ARR[Wleft_bin],Q2gen)*(getEpsT(2.445,W_ARR[Wleft_bin],Q2gen)+1));
  // sigma_wleft[4] = sigma_wleft[4]*
  // func_sigma_t(Q2gen,Wleft_bin)/SIGMA_T_INT_RIPANI[Wleft_bin];
  ...
  for (Short_t i = 0; i < 6; i++)
  {
    ...
    sigma_final[i] =
        sigma_final[i] * (sigma_wright[i] * fabs(W_ARR[Wleft_bin] - Wgen) +
                          sigma_wleft[i] * fabs(W_ARR[Wright_bin] - Wgen));
  }
  ...
  if ((Wgen >= 1.6125) && (Wgen <= 1.8125))
  {
    ...
    // sigma_t_wright_gol =
    // sigma_t_wright_gol*func_sigma_t(Q2gen,Wright_bin)/func_sigma_t(0.005,Wright_bin);
    // sigma_t_wleft_gol =
    // sigma_t_wleft_gol*func_sigma_t(Q2gen,Wleft_bin)/func_sigma_t(0.005,Wleft_bin);
    //}
    // if ((Wgen>1.6125)&&(Wgen<=1.7125)) {
    sigma_t_wright_gol = sigma_t_wright_gol * func_sigma_t(Q2gen, Wright_bin) /
                         A_tmp[Wright_bin - 8];
    sigma_t_wleft_gol = sigma_t_wleft_gol * func_sigma_t(Q2gen, Wleft_bin) /
                        A_tmp[Wleft_bin - 8];
    //}
    ...
    sigma_t_gol =
        sigma_t_gol * (sigma_t_wright_gol * fabs(W_ARR[Wleft_bin] - Wgen) +
                       sigma_t_wleft_gol * fabs(W_ARR[Wright_bin] - Wgen));

    // Mixing Ripani and Golovach cross sections
    sigma_t_final = (Q2gen - 0.0003) / 0.65 * sigma_t_final +
                    (0.65 - Q2gen) / 0.65 * sigma_t_gol;
  }
