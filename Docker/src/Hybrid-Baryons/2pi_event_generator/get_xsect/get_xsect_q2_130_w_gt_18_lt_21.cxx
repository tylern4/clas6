...
void get_xsect_q2_130_w_gt_18_lt_21(
    Float_t E_beam, Float_t Q2gen, Float_t Wgen, Float_t s12gen, Float_t s23gen,
  ...
  Short_t s12left_wleft_bin =
      getsbin(Wleft_bin, s12gen, S12_ARR_RIP2[11][Wleft_bin],
              S12_ARR_RIP2[0][Wleft_bin]);
  Short_t s12right_wleft_bin = s12left_wleft_bin + 1;

  Short_t s12left_wright_bin =
      getsbin(Wright_bin, s12gen, S12_ARR_RIP2[11][Wright_bin],
              S12_ARR_RIP2[0][Wright_bin]);
  Short_t s12right_wright_bin = s12left_wright_bin + 1;

  Short_t s23left_wleft_bin =
      getsbin(Wleft_bin, s23gen, S23_ARR_RIP2[11][Wleft_bin],
              S23_ARR_RIP2[0][Wleft_bin]);
  Short_t s23right_wleft_bin = s23left_wleft_bin + 1;

  Short_t s23left_wright_bin =
      getsbin(Wright_bin, s23gen, S23_ARR_RIP2[11][Wright_bin],
              S23_ARR_RIP2[0][Wright_bin]);
  ...
  for (Short_t i = 0; i < 6; i++) {
    interpol_rip2(4, Wright_bin, s12left_wright_bin, s12right_wright_bin,
    ...
    interpol_rip2(4, Wleft_bin, s12left_wleft_bin, s12right_wleft_bin,
  ...
  if ((Q2gen > 0.0002) && (Q2gen <= 0.65)) {
    ...
    sigma_t_gol =
        sigma_t_gol * (sigma_t_wright_gol * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
                       sigma_t_wleft_gol * fabs(W_ARR_RIP2[Wright_bin] - Wgen));
    ...
    sigma_final[0] =
        sigma_final[0] * (sigma_wright[0] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
                          sigma_wleft[0] * fabs(W_ARR_RIP2[Wright_bin] - Wgen));
    ...
    // if ((Q2gen > 0.3)&&(Q2gen <=0.65)&&(E_beam<2.45)) sigma_t_final
    // =(-4./7.*Q2gen+41./35.)*sigma_t_final;  if ((Q2gen > 0.3)&&(Q2gen
    // <=0.65)&&(E_beam>2.45)&&(E_beam<3.)) sigma_t_final
    // =(-2./7.*Q2gen+38./35.)*sigma_t_final;

    if ((Q2gen > 0.4) && (Q2gen <= 0.65) && (E_beam < 2.45))
      sigma_t_final = (-0.8 * Q2gen + 1.32) * sigma_t_final;
    if ((Q2gen > 0.4) && (Q2gen <= 0.65) && (E_beam > 2.45) && (E_beam < 3.))
      sigma_t_final = (-0.4 * Q2gen + 1.16) * sigma_t_final;
  ...
  if ((Q2gen >= 0.95) && (Q2gen <= 1.3)) {
    ...
    sigma_l_final =
        sigma_l_final * (sigma_wright[1] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
                         sigma_wleft[1] * fabs(W_ARR_RIP2[Wright_bin] - Wgen));
  }
  ...
  for (Short_t i = 2; i < 6; i++) {
    ...
    sigma_final[i] =
        sigma_final[i] * (sigma_wright[i] * fabs(W_ARR_RIP2[Wleft_bin] - Wgen) +
                          sigma_wleft[i] * fabs(W_ARR_RIP2[Wright_bin] - Wgen));
  }
