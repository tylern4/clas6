...
// 1 - using auxiliary functions (getWbin_GOL, getsbin_GOL, getanglebin_GOL) we
// identify the left and right point for each variable generated  (according to
// Golovach binning)  2 - then we are doing 4d-interpolation for each Wleft_bin
// and Wright_bin and obtain sigma_wright_gol and sigma_wleft_gol

void get_xsect_golovach(Float_t Wgen, Float_t s12gen, Float_t s23gen,
                        Float_t thetagen, Float_t alphagen, Short_t &Wleft_bin,
                        Float_t &sigma_wright_gol, Float_t &sigma_wleft_gol) {
  // using auxiliary functions (getWbin_GOL, getsbin_GOL, getanglebin_GOL) we
  // identify the left and right point for each variable generated  (according
  // to Golovach binning)
  Wleft_bin = getWbin_GOL(Wgen);
  Short_t Wright_bin = Wleft_bin + 1;

  // cout << Wgen << " "<< Wleft_bin<< " "<< W_ARR_GOL[Wleft_bin] <<" "<<
  // W_ARR_GOL[Wright_bin]<< "\n";

  Short_t s12left_wleft_bin = getsbin_GOL(s12gen, S12_ARR_GOL[15][Wleft_bin],
                                          S12_ARR_GOL[0][Wleft_bin]);
  Short_t s12right_wleft_bin = s12left_wleft_bin + 1;

  // cout <<s12left_wleft_bin <<" "<<s12gen<<"
  // "<<S12_ARR_GOL[s12left_wleft_bin][Wleft_bin]<< " "<<
  // S12_ARR_GOL[s12right_wleft_bin][Wleft_bin]<<"\n";
  Short_t s12left_wright_bin = getsbin_GOL(s12gen, S12_ARR_GOL[15][Wright_bin],
                                           S12_ARR_GOL[0][Wright_bin]);
  Short_t s12right_wright_bin = s12left_wright_bin + 1;

  Short_t s23left_wleft_bin = getsbin_GOL(s23gen, S23_ARR_GOL[15][Wleft_bin],
                                          S23_ARR_GOL[0][Wleft_bin]);
  Short_t s23right_wleft_bin = s23left_wleft_bin + 1;

  Short_t s23left_wright_bin = getsbin_GOL(s23gen, S23_ARR_GOL[15][Wright_bin],
                                           S23_ARR_GOL[0][Wright_bin]);
  ...
  // cout << s23left_wleft_bin <<" "<< Wgen << " "<<Wleft_bin<< " " << s23gen <<
  // " "<< S23_ARR_GOL[s23left_wleft_bin][Wleft_bin] << " "<<
  // S23_ARR_GOL[s23right_wleft_bin][Wleft_bin] << "\n";

  // then we are doing 4d-interpolation for each Wleft_bin and Wright_bin and
  // obtain sigma_wright_gol and sigma_wleft_gol
  interpol_golovach(Wright_bin, s12left_wright_bin, s12right_wright_bin,
                    s23left_wright_bin, s23right_wright_bin, thetaleft_bin,
                    thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                    s23gen, thetagen, alphagen, sigma_wright_gol);

  interpol_golovach(Wleft_bin, s12left_wleft_bin, s12right_wleft_bin,
                    s23left_wleft_bin, s23right_wleft_bin, thetaleft_bin,
                    thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                    s23gen, thetagen, alphagen, sigma_wleft_gol);

  // cout <<Wgen<<" "<< sigma_wright_gol << " "<<sigma_wleft_gol<<"\n";

  // cout << Wgen << " "<< s12left_wright_bin << " "<<s12right_wright_bin<<"
  // "<<S12_ARR_GOL[s12right_wright_bin][Wright_bin] << "
  // "<<s23left_wright_bin<< " "<< s23right_wright_bin<< " "<<
  // sigma_wright_gol<<"\n";  cout << Wgen << " "<< s12left_wleft_bin << "
  // "<<s12right_wleft_bin<<" "<<S12_ARR_GOL[s12right_wleft_bin][Wleft_bin]<< "
  // "<<s23left_wleft_bin<< " "<< s23right_wleft_bin<< " "<<
  // sigma_wleft_gol<<"\n";
  ...
  // sigma_t_gol =
  // sigma_t_gol*(sigma_wright_gol*fabs(W_ARR_GOL[Wleft_bin]-Wgen)+sigma_wleft_gol*fabs(W_ARR_GOL[Wright_bin]-Wgen));
  // cout << sigma_t_gol << " "<< sigma_wright_gol << " "<<
  // sigma_wleft_gol<<"\n";
  return;
}
