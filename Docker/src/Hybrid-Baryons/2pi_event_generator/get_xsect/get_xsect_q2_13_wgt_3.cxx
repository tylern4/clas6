...
Short_t getsbin_wgt_3(Float_t sgen, Float_t Smax, Float_t Smax2, Float_t Smin) {
  // if ((sgen>=Smin)&&(sgen<=Smax2)) return int((sgen-Smin)/((Smax2 -
  // Smin)/5.));  if ((sgen>=Smax2)&&(sgen<=Smax)) return 5 +
  // int((sgen-Smax2)/((Smax - Smax2)/10.));  if ((sgen>=Smin)&&(sgen<=Smax2))
  // return int((sgen-Smin)/((Smax2 - Smin)/13.));  if
  // ((sgen>=Smax2)&&(sgen<=Smax)) return 13 + int((sgen-Smax2)/((Smax -
  // Smax2)/2.));
  if ((sgen >= Smin) && (sgen <= Smax2))
    return int((sgen - Smin) / ((Smax2 - Smin) / 11.));
...
void get_xsect_q2_13_wgt_3(Float_t Q2gen, Float_t Wgen, Float_t s12gen,
  ...
  Short_t s12left_wleft_bin =
      getsbin_wgt_3(s12gen, S12_ARR_gt_3[15][Wleft_bin],
                    S12_ARR_gt_3[11][Wleft_bin], S12_ARR_gt_3[0][Wleft_bin]);
  Short_t s12right_wleft_bin = s12left_wleft_bin + 1;

  Short_t s12left_wright_bin =
      getsbin_wgt_3(s12gen, S12_ARR_gt_3[15][Wright_bin],
                    S12_ARR_gt_3[11][Wright_bin], S12_ARR_gt_3[0][Wright_bin]);
  Short_t s12right_wright_bin = s12left_wright_bin + 1;

  Short_t s23left_wleft_bin =
      getsbin_wgt_3(s23gen, S23_ARR_gt_3[15][Wleft_bin],
                    S23_ARR_gt_3[11][Wleft_bin], S23_ARR_gt_3[0][Wleft_bin]);
  Short_t s23right_wleft_bin = s23left_wleft_bin + 1;

  Short_t s23left_wright_bin =
      getsbin_wgt_3(s23gen, S23_ARR_gt_3[15][Wright_bin],
                    S23_ARR_gt_3[11][Wright_bin], S23_ARR_gt_3[0][Wright_bin]);
  ...
  for (Short_t j = 0; j < 6; j++) {
    interpol_q2_13_wgt_3(4, Wright_bin, s12left_wright_bin, s12right_wright_bin,
                         s23left_wright_bin, s23right_wright_bin, thetaleft_bin,
                         thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                         s23gen, thetagen, alphagen, sigma_wright[j], j);

    interpol_q2_13_wgt_3(4, Wleft_bin, s12left_wleft_bin, s12right_wleft_bin,
                         s23left_wleft_bin, s23right_wleft_bin, thetaleft_bin,
                         thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                         s23gen, thetagen, alphagen, sigma_wleft[j], j);
  }

  interpol_phot_wgt_3(4, Wright_bin, s12left_wright_bin, s12right_wright_bin,
                      s23left_wright_bin, s23right_wright_bin, thetaleft_bin,
                      thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                      s23gen, thetagen, alphagen, sigma_wright_ph);

  interpol_phot_wgt_3(4, Wleft_bin, s12left_wleft_bin, s12right_wleft_bin,
                      s23left_wleft_bin, s23right_wleft_bin, thetaleft_bin,
                      thetaright_bin, alphaleft_bin, alpharight_bin, s12gen,
                      s23gen, thetagen, alphagen, sigma_wleft_ph);
  ...
  if ((Q2gen > 0.0002) && (Q2gen <= 0.65)) {
    ...
    sigma_t_fin_ph =
        sigma_t_fin_ph * (sigma_wright_ph * fabs(W_ARR_gt_3[Wleft_bin] - Wgen) +
                          sigma_wleft_ph * fabs(W_ARR_gt_3[Wright_bin] - Wgen));

    sigma_t_fin_ph =
        sigma_t_fin_ph * func_sigma_t(Q2gen, 16) / func_sigma_t(0.003, 16);

    sigma_final[0] = 1. / fabs(W_ARR_gt_3[Wright_bin] - W_ARR_gt_3[Wleft_bin]);
    sigma_final[0] =
        sigma_final[0] * (sigma_wright[0] * fabs(W_ARR_gt_3[Wleft_bin] - Wgen) +
                          sigma_wleft[0] * fabs(W_ARR_gt_3[Wright_bin] - Wgen));

    sigma_t_final = (Q2gen - 0.0003) / 1.3 * 3. * sigma_final[0] +
                    (1.3 - Q2gen) / 1.3 * sigma_t_fin_ph;
  }

  if ((Q2gen >= 0.65) && (Q2gen <= 1.3)) {
    // Q2-scale for photon point
    sigma_wright_ph =
        sigma_wright_ph * func_sigma_t(0.65, 16) / func_sigma_t(0.003, 16);
    sigma_wleft_ph =
        sigma_wleft_ph * func_sigma_t(0.65, 16) / func_sigma_t(0.003, 16);
    ...
    sigma_t_fin_ph = (1.3 - 0.65) / 1.3 * sigma_t_fin_ph +
  ...
  if ((Q2gen >= 0.95) && (Q2gen <= 1.3)) {
    ...
    sigma_l_final =
        sigma_l_final * (sigma_wright[1] * fabs(W_ARR_gt_3[Wleft_bin] - Wgen) +
                         sigma_wleft[1] * fabs(W_ARR_gt_3[Wright_bin] - Wgen));
  }
  ...
  for (Short_t i = 2; i < 6; i++) {
    ...
    sigma_final[i] =
        sigma_final[i] * (sigma_wright[i] * fabs(W_ARR_gt_3[Wleft_bin] - Wgen) +
                          sigma_wleft[i] * fabs(W_ARR_gt_3[Wright_bin] - Wgen));
  }
  ...
  // cout << sigma_wright[1]<< " "<< sigma_wleft[1]<<" "<< Wleft_bin<< " "<<
  // s12left_wleft_bin<< " "<< s23left_wleft_bin<< "\n";
}
