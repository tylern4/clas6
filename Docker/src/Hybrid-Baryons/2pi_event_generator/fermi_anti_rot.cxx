...
//----------------------------------------------------
// This subroutine performes the quasiLab-->Lab transformation, where quasiLab
// is the system, where the initial proton is at rest, while the initial
// electron moves along the Z-axis. The transformation is performed via the
// sequence of auxiliary systems: quasiLab(System 3)-->System 2-->System
// 1-->Lab.  As an output it gives the four-momenta of the final hadrons (P4_1,
// P4_2, and P4_3) in the Lab frame.
...
void fermi_anti_rot(Float_t W_ferm, Float_t Q2, Float_t E_beam,
  ...
  // For the testing purposes let's construct the four-momenta of the incoming
  // and scattered electons in the quasiLab frame
  ...
  P4_E_prime_qLab.SetXYZT(E_E_prime_new2 * cos(phi_e) * sin(Theta_e_prime2),
                          E_E_prime_new2 * sin(phi_e) * sin(Theta_e_prime2),
                          E_E_prime_new2 * cos(Theta_e_prime2), E_E_prime_new2);

  // Defining of the spatial angles of the Fermi momentum
  Float_t theta_fermi, phi_fermi;

  theta_fermi = acos(pz_fermi / sqrt(px_fermi * px_fermi + py_fermi * py_fermi +
                                     pz_fermi * pz_fermi));
  phi_fermi =
      acos(fabs(px_fermi) / sqrt(px_fermi * px_fermi + py_fermi * py_fermi));
  ...
  beta = sqrt(px_fermi * px_fermi + py_fermi * py_fermi + pz_fermi * pz_fermi) /
         sqrt(MP * MP + px_fermi * px_fermi + py_fermi * py_fermi +
              pz_fermi * pz_fermi);
  ...
  // Now, the four momenta of all final hadrons as well as the initial
  // (P4_Eini_qLab) and scatteres electrons (P4_E_prime_qLab) are written in the
  // usual Lab frame!
  ...
  // For the testing purposes let's compare the obtained four-momenta of the
  // initial (P4_Eini_qLab) and scatteres electrons (P4_E_prime_qLab) with those
  // already exsisted in the Lab frame.
  ...
  if ((fabs(P4_E_prime_qLab[0] - P4_E_prime_Lab[0]) > 0.0001) ||
      (fabs(P4_E_prime_qLab[1] - P4_E_prime_Lab[1]) > 0.0001) ||
      (fabs(P4_E_prime_qLab[2] - P4_E_prime_Lab[2]) > 0.0001) ||
      (fabs(P4_E_prime_qLab[2] - P4_E_prime_Lab[2]) > 0.0001))
    cout << "ALARM! Wrong Lab-->quasiLab-->Lab momenta transformation! \n";

  if ((fabs(P4_Eini_qLab[0] - P4_Eini_Lab[0]) > 0.0001) ||
      (fabs(P4_Eini_qLab[1] - P4_Eini_Lab[1]) > 0.0001) ||
      (fabs(P4_Eini_qLab[2] - P4_Eini_Lab[2]) > 0.0001) ||
      (fabs(P4_Eini_qLab[2] - P4_Eini_Lab[2]) > 0.0001))
    cout << "ALARM! Wrong Lab-->quasiLab-->Lab momenta transformation! \n";
}
