...
void rot(Float_t Q2, Float_t E_beam, TLorentzVector P4_E_fin,
         TLorentzVector P4_1, TLorentzVector P4_2, TLorentzVector P4_3,
  ...
  // cout << E_beam<< "  " << MP<< "\n";
  // cout << "qqq2"<<P4_E_fin [0] << " "<< P4_E_fin [1] << " "<< P4_E_fin [2]
  // <<" "<< P4_E_fin [3] << "\n";
  ...
  a_gamma = sqrt(
      1000000000000. /
      (1000000000000. - (1000000. * (P4_1.Vect().Unit() * V3_anti_z)) *
                            (1000000. * (P4_1.Vect().Unit() * V3_anti_z))));
  // cout << a_gamma<<" "<<(1000000.*(P4_1.Vect().Unit() *
  // V3_anti_z))*(1000000.*(P4_1.Vect().Unit() * V3_anti_z))<<" p1 \n";
  b_gamma = -(P4_1.Vect().Unit() * V3_anti_z) * a_gamma;

  Vect3_gamma = a_gamma * V3_anti_z + b_gamma * P4_1.Vect().Unit();

  a_beta = sqrt(1000000000000. /
                (1000000000000. -
                 (1000000. * (P4_1.Vect().Unit() * P4_2.Vect().Unit())) *
                     (1000000. * (P4_1.Vect().Unit() * P4_2.Vect().Unit()))));

  // cout << a_beta<<" "<<(1000000.*(P4_1.Vect().Unit() *
  // P4_2.Vect().Unit()))*(1000000.*(P4_1.Vect().Unit() * P4_2.Vect().Unit()))
  // <<" p3 \n";
  ...
  if (Vect3_gamma.Cross(Vect3_beta) * P4_1.Vect() < 0.)
    alpha_hadr = 2. * M_PI - alpha_hadr;
  ...
  if (Vect3_gamma.Cross(Vect3_beta) * P4_3.Vect() < 0) alpha_hadr = 2.*M_PI -
  alpha_hadr;
  */
  /*
  ///3
  ...
  if (Vect3_gamma.Cross(Vect3_beta) * P4_2.Vect() < 0) alpha_hadr = 2.*M_PI -
  alpha_hadr;
  */
  return;
}
