...
// out_file_close() - it closes all outputs including the root tree with the
// cross section values  hist_write() - it writes the histogramms into the
// auxiliary root-file
int out_file_close() {
#ifdef BOS
  ...
  // III. Writing root tree with the cross section values into the root-file by
  // default
...
void hist_write() {
  // Dividing by Delta cos to obtain d(-cos theta) distributions for 1.225 < W
  // < 2.15 GeV.
  for (Int_t ik = 0; ik <= 36; ik++) {
    for (Int_t ij = 1; ij <= 100; ij++) {
      h_odn_theta_2[ik]->SetBinContent(
          ij, h_odn_theta[ik]->GetBinContent(ij) /
                  (cos((h_odn_theta[ik]->GetBinLowEdge(ij))) -
                   cos((h_odn_theta[ik]->GetBinLowEdge(ij) +
                        h_odn_theta[ik]->GetBinWidth(ij)))));
    }
  }

  // Dividing by Delta cos to obtain d(-cos theta) distributions for 2.1625 < W
  // < 3.0375 GeV.
  for (Int_t ik = 0; ik < 18; ik++) {
    for (Int_t ij = 1; ij <= 100; ij++) {
      h_odn_wwide_theta_2[ik]->SetBinContent(
          ij, h_odn_wwide_theta[ik]->GetBinContent(ij) /
                  (cos((h_odn_theta[ik]->GetBinLowEdge(ij))) -
                   cos((h_odn_theta[ik]->GetBinLowEdge(ij) +
                        h_odn_theta[ik]->GetBinWidth(ij)))));
    }
  ...
  // Dividing by Delta cos to obtain d(-cos theta) distributions for 3.0875 < W
  // < 4.5375 GeV.
  for (Int_t ik = 0; ik < 15; ik++) {
    for (Int_t ij = 1; ij <= 100; ij++) {
      h_odn_wgt3_theta_2[ik]->SetBinContent(
          ij, h_odn_wgt3_theta[ik]->GetBinContent(ij) /
                  (cos((h_odn_theta[ik]->GetBinLowEdge(ij))) -
                   cos((h_odn_theta[ik]->GetBinLowEdge(ij) +
                        h_odn_theta[ik]->GetBinWidth(ij)))));
    }
