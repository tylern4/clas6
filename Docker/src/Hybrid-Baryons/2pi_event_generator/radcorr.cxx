...
// Calculating lengthes of the target and i/f windows using input-file
// information
Float_t T_targ;
Float_t T_wi;
Float_t T_wf;
...
Float_t Gamma_V_dE_dOm(Float_t E_beam, Float_t Wgen, Float_t Q2gen) {
  ...
  Float_t eps_t = 1. / (1. + 2. * (1. + E_gamma_lab * E_gamma_lab / Q2gen) *
                                 tan(theta_el / 2.) * tan(theta_el / 2.));
  Float_t GV = _GV;
...
// Auxiliary B-function according to Mo&Tsai Appendix A, furmalae A.4 and A.5
// page 226------
Float_t BZFUN(Float_t Z) {
  Float_t logz = log(Z);
  // Float_t log_183 = log(183.);
  // Float_t log_1440 = log(1440.);

  Float_t log_1440 = 7.272398;
  Float_t log_183 = 5.209486;

  Float_t xi = (log_1440 - 2. * logz / 3.) / (log_183 - logz / 3.);

  return 4. / 3. * (1. + ((Z + 1.) / (Z + xi)) / (log_183 - logz / 3.) / 9.);
}
...
// This subroutine calculates the first term of the formula (IV.1) Mo&Tsai
// (page 213)  It includes:
//* calculation of two factors Delta_r and Delta_t (see section IV.A Mo and Tsai
// pages 213-214)
//* calculation of 2dim linearly interpolated intergated 2pi cross-section (in
// W-Q2 variables) including calculation sigma_t and sigma_l separately and
// their  combonation with eps_L
//*  multiplying by the Gamma_V_dE_dOm - virtual photon flux for the case (E_f,
// Omega variables)
Float_t s1_radsoft(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  Delta_t =
      (Bwi * T_wi + 0.5 * Btarg * T_targ) * log(E_beam / delta) +
      (Bwf * T_wf + 0.5 * Btarg * T_targ) * log((E_beam - E_gamma_lab) / delta);
  Delta_t = -1. * Delta_t;

  Delta_r = 28. / 9. - (13. / 6.) * log(Q2gen / Me / Me) +
            (log(E_beam / delta) + log((E_beam - E_gamma_lab) / delta)) *
                (log(Q2gen / Me / Me) - 1.) -
            Spence(-E_gamma_lab / (E_beam - E_gamma_lab)) -
            Spence(E_gamma_lab / E_beam);
...
Float_t Omega_max_ini(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  // Float_t E_beam_min = (MPIM*MPIM + 2.*MP*MPIM + 2.*MP*Ep)/(2.*MP - 2.*Ep*(1.
  // - cos(theta_el)));

  // For the case of 2pi-treshold (see Mo&Tsai formula (A.17)&(A.18))
  Float_t E_beam_min = (2. * MPIM * MPIM + 2. * MP * MPIM + MP * Ep) /
                       (MP - Ep * (1. - cos(theta_el)));

  return E_beam - E_beam_min;
}

Float_t Rho_s_radhard(Float_t E_beam, Float_t Q2gen, Float_t Wgen,
                      Float_t omega) {
  Float_t Btarg = BZFUN(Targ_Z);
  Float_t Bwi = BZFUN(Twi_Z);
  Float_t Bwf = BZFUN(Twf_Z);
  ...
  Float_t ts =
      alpha / M_PI * (0.5 * (1. + xs * xs) * log(Q2gen / Me / Me) - xs);
...
Float_t DSIMPS_s2() {
  ...
  for (Int_t i = 2; i < Npoints; i += 2) {
    ...
    Integral += (ARR_r_radhardini[i - 2] + 4. * ARR_r_radhardini[i - 1] +
                 ARR_r_radhardini[i]) *
                h;
  }
...
// This subroutine calculates the second term of the formula (IV.1) Mo&Tsai
// (page 213)  It corresponds to the case when initial electron emits photon, so
// the beam energy changes  The final electron is asumed unchanged!  It
// includes:
//* determination of the limits of integration. Intergation is performed over
// the possible photon energy,  so the lower limit is delta and the upper is
// calculated by subroutine Omega_max_ini according to the  reaction treshold -
// see Mo&Tsai formula (A.17)&(A.18)
...
//-------*calculating "new" W and Q2 values assuming the final electron
// unchanged!
//-------*picking the cross-section for "new" W and Q2
//-------*calculating the rest of the integrand of the second term of the
// formula (IV.1) Mo&Tsai
//--------by using the subroutin Rho_s_radhard
//*performing integration of the function Cross-section*Rho_s_radhard defined on
// the grid given by loop
Float_t s2_radhardini(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  for (Int_t i = 1; i <= Npoints; i++) {
    ...
    if ((w_new > 1.2375) && (w_new < 4.5375)) {
      ...
      sec_total = sec_test_t +
                  getEpsL(E_beam - omega_current, w_new, q2_new) * sec_test_l;
    }

    sec_total =
        sec_total * Gamma_V_dE_dOm(E_beam - omega_current, w_new, q2_new);
    ARR_r_radhardini[i - 1] =
        Rho_s_radhard(E_beam, Q2gen, Wgen, omega_current) * sec_total;
  }
...
Float_t Omega_max_fin(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  // Float_t E_p_max = (2.*MP*E_beam - 2.*MP*MPIM -MPIM*MPIM)/(2.*MP
  // + 2.*E_beam*(1. - cos(theta_el)));

  // For the case of 2pi-treshold (see Mo&Tsai formula (A.17)&(A.19))
  Float_t E_p_max = (MP * E_beam - 2. * MP * MPIM - 2. * MPIM * MPIM) /
                    (MP + E_beam * (1. - cos(theta_el)));
...
//-------------------------------------------------------------------------
Float_t Rho_p_radhard(Float_t E_beam, Float_t Q2gen, Float_t Wgen,
                      Float_t omega) {
  Float_t Btarg = BZFUN(Targ_Z);
  Float_t Bwi = BZFUN(Twi_Z);
  Float_t Bwf = BZFUN(Twf_Z);
  ...
  Float_t tp =
      alpha / M_PI * (0.5 * (1. + xp * xp) * log(Q2gen / Me / Me) - xp);
...
Float_t DSIMPS_s3() {
  ...
  for (Int_t i = 2; i < Npoints; i += 2) {
    ...
    Integral += (ARR_r_radhardfin[i - 2] + 4. * ARR_r_radhardfin[i - 1] +
                 ARR_r_radhardfin[i]) *
                h;
  }
...
// This subroutine calculates the third term of the formula (IV.1) Mo&Tsai (page
// 213)  It corresponds to the case when final electron emits photon, so the
// energy of the final electron changes  The initial electron is asumed
// unchanged! It includes:
//* determination of the limits of integration. Intergation is performed over
// the possible photon energy,  so the lower limit is delta and the upper is
// calculated by subroutine Omega_max_fin according to the  reaction treshold -
// see Mo&Tsai formula (A.17)&(A.18)
//* the loop over the possible photon energies from min to max
//------for each loop iteration we are doing the following:
//-------*calculating "new" energy of the final electron
//-------*calculating "new" W and Q2 values assuming the initial electron
// unchanged!
//-------*picking the cross-section for "new" W and Q2
//-------*calculating the rest of the integrand of the third term of the formula
//(IV.1) Mo&Tsai
//--------by using the subroutin Rho_p_radhard
//*performing integration of the function Cross-section*Rho_p_radhard defined on
// the grid given by loop
Float_t s3_radhardfin(Float_t E_beam, Float_t Q2gen, Float_t Wgen) {
  ...
  for (Int_t i = 1; i <= Npoints; i++) {
    ...
    ARR_r_radhardfin[i - 1] =
        Rho_p_radhard(E_beam, Q2gen, Wgen, omega_current) * sec_total;
  }
...
void radcorr(Float_t R, Double_t R_ini, Double_t R_fin, Float_t E_beam,
  ...
  // cout << "s1 = "<< s1_radsoft(E_beam, 0.9,1.35)<< ", s2 = "<<
  // s2_radhardini(E_beam,0.9,1.35)<< ", s3 =
  // "<<s3_radhardfin(E_beam,0.9,1.35)<<"\n";
  ...
  if ((R >= s1 / (s1 + s2 + s3)) && (R < (s1 + s2) / (s1 + s2 + s3))) {
    TH1F *h_radhardini =
        new TH1F("h_radhardini", "h_radhardini", 800, ARR_e_radhardini[0],
                 ARR_e_radhardini[Npoints - 1]);

    for (Int_t i = 0; i < Npoints - 1; i++) {
      h_radhardini->SetBinContent(
          i + 1, (ARR_r_radhardini[i] + ARR_r_radhardini[i + 1]) / 2.);
    }
    // R_ini is a random number from 0 to 1. It is an input parameter.
    R1[0] = R_ini;

    h_radhardini->GetQuantiles(1, eran_hardini, R1);
    // cout << eran_hardini[0]<<" "<< ARR_e_radhardini[0]<<"
    // "<<ARR_e_radhardini[Npoints-1] << "\n";
  ...
  if ((R >= (s1 + s2) / (s1 + s2 + s3)) && (R < 1.)) {
    TH1F *h_radhardfin =
        new TH1F("h_radhardfin", "h_radhardfin", 800, ARR_e_radhardfin[0],
                 ARR_e_radhardfin[Npoints - 1]);

    for (Int_t i = 0; i < Npoints - 1; i++) {
      h_radhardfin->SetBinContent(
          i + 1, (ARR_r_radhardfin[i] + ARR_r_radhardfin[i + 1]) / 2.);
    }
