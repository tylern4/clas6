...
// We assume as global variables: MP - initial proton mass (is taken from
// pdg-table), E_beam - beam energy (is taken from input file).

// Here all derivations are performed via special root functions. If you need
// further level of explanation please see anti_rot_explanation, where all
...
void anti_rot(Float_t W, Float_t Q2, Float_t phi_el, Float_t E_beam, Float_t M12, Float_t M23, Float_t theta_hadr,
  ...
  // Float_t en_1,en_2,en_3,mag_1, mag_2, mag_3, th_1, th_2, th_3, ph_1, ph_2,
  // ph_3;
  ...
  // At this stage only magnitudes of 3-momenta and energy components of
  // 4-momenta of final hadrons are defined (they are the same for ANY axes
  // orientations). To define 4-momenta completely we need to calculate their
  // spatial angles for SOME axis orientation.

  // Angles calculation in the system where Z-axis along particle #1, X
  // perpendicular to Z and situated in a-b-1 plane (plane A).  Two remaining
  // final hadrons (#2 & #3) are located in plane B.  We choose such a frame,
  // because in this frame azimuthal angles of remaining final hadrons
  // (phi-angles) are equal (or derived via) given alpha-angle that is defined
  // as angle between planes A and B.  Here we simultaneously set defined angles
  // (theta and phi) as angles of 4-momenta of final hadrons that means we
  ...
  */
  ...
  // cout << P4_3 [0] << "\n";
  // Now we need to rotate our auxiliary coordinate system to the usual
  // CMS-axes-orientation: Z-axis along particle a (virtual photon) and X-axis
  // is in ee'-plane  For that purpose we need to do two rotations of coordinate
  // axes:

  // 1 - rotate Z-axis with the angle theta_hadr in the XZ-plane (around Y-axis)
  // to set  Z-axis along particle a (virtual photon). After this rotation
  ...
  // Here we transfom our final hadrons 4-momenta into the system described
  // above
  ...
  // After that our final hadrons 4-momenta are defined in the usual CMS-system
  // that that will be in the LAB frame after the boost

  // cout <<"qqq1 "<< P4_1[0]<< " " << P4_1[1]<< " " <<P4_1[2]<< " " <<P4_1[3]<<
  // "\n ";

  // Now we need to perform boost to find ourselves in the LAB-frame.
  // beta here is beta of transformation from LAB-system to CMS and since root
  // function requires to use -beta we have no minus sign in the formulae below
  ...
  // Now we are in the LAB-frame (let's mark it frame 2) with Z2 - along the
  // virtual photon, X2 - in the ee'-plane and Y2 - perpendicular to the
  // e'-plane.  The only thing left is to rotate our coordinate axes into usual
  // LAB-frame-axes-orientation (lets call it frame 1): Z1-axis along the beam,
  // Y1 - up, X1 - to the south.  For that purpose we will find coordinates of
  // unit vectors along the axes of frame 2 in frame 1.

  TVector3 P3_EL, P3_G, P3_X, P3_Y;

  // We define 3-vector of scattered electron in LAB-frame 1 (theta is from the
  // formula for Q2 in the LAB frame: Q2 = 2E_beamE_e'(1-cos(theta_e')))
  P3_EL.SetXYZ(0., 0., 1.);
  ...
  // Now unit vectors of frame 2 are written in the frame 1. We define rotation
