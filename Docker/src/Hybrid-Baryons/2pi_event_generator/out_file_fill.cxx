...
// out_file_fill(...) - it fills the desired output file with the generated info
// + the root-tree with weights.  hist_fill(..) - it fills the histogram, which
// are needed for the illustrative and testing puposes.

void out_file_fill(int i, Float_t sigma, Float_t W, Float_t Q2,
                   TLorentzVector& P4_E, TLorentzVector& P4_1,
                   TLorentzVector& P4_2, TLorentzVector& P4_3, Float_t z_EL,
                   Float_t x_EL, Float_t y_EL) {
#ifdef BOS
  ...
  if ((flag_bos == 1) || (flag_bos == 2)) {
    ...
    clasHEAD_t* HEAD;
    if ((HEAD =
             (clasHEAD_t*)makeBank(&bcs_, "HEAD", 0, sizeof(head_t) / 4, 4))) {
    ...
    clasMCEV_t* MCEV;
    if ((MCEV =
             (clasMCEV_t*)makeBank(&bcs_, "MCEV", 0, sizeof(mcev_t) / 4, 1))) {
    ...
    if (flag_bos == 1) {
      clasMCTK_t* MCTK;
      if ((MCTK = (clasMCTK_t*)makeBank(&bcs_, "MCTK", 0, sizeof(mctk_t) / 4,
                                        4))) {
        // electron
      ...
      clasMCVX_t* MCVX;
      if ((MCVX = (clasMCVX_t*)makeBank(&bcs_, "MCVX", 0, sizeof(mcvx_t) / 4,
                                        1))) {
        MCVX->mcvx[0].x = x_EL;
        MCVX->mcvx[0].y = y_EL;
        MCVX->mcvx[0].z = z_EL;
    ...
    if (flag_bos == 2) {
      clasPART_t* PART;
      if ((PART = (clasPART_t*)makeBank(&bcs_, "PART", 0, sizeof(part_t) / 4,
                                        4))) {
        // electron
        PART->part[0].pid = 3.;
  ...
  if (flag_lund == 1) {
    out_lund_stream << "4  1.  1.  0  0  " << i << "  0  " << std::fixed
                    << std::setprecision(6) << W << "  " << Q2 << "  " << sigma
                    << "\n";
  ...
  // III. Filling the root tree with the cross section weights and components of
  // the fermi momentum
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// This subroutine fills the histogram, which are needed for illustrative and
// testing puposes.
...
void hist_fill(Float_t E_beam, Float_t W, Float_t W_old, Float_t Q2,
  ...
  P4_Pini_fermi.SetXYZT(px_fermi, py_fermi, pz_fermi,
                        sqrt(MP * MP + px_fermi * px_fermi +
                             py_fermi * py_fermi + pz_fermi * pz_fermi));
  P4_Eini.SetXYZT(0., 0., E_beam, E_beam);

  // P4_Pini - the four-momentum of the initial proton at rest
  // P4_Eini - the four-momentum of the incomimg electron in the Lab frame, no
  // rad eff  P4_E_prime - the four-momentum of the scattered electron in the
  // Lab frame, no rad eff  P4_PIP, P4_Pfin, P4_PIM - the four-momenta of the
  // final hadrons in the Lab frame

  // P4_Pini_fermi - the four-momentum of the moving initial proton

  // P4_Eini_new - the four-momentum of the incomimg electron in the Lab frame,
  // with rad eff  P4_E_prime_new - the four-momentum of the scattered electron
  // in the Lab frame, with rad eff

  P4_0_miss = P4_Pini + P4_Eini - P4_E_prime - P4_PIP - P4_Pfin - P4_PIM;

  if ((flag_radmod == 0) && (flag_fermi == 1))
    P4_0_miss_2 =
        P4_Pini_fermi + P4_Eini - P4_E_prime - P4_PIP - P4_Pfin - P4_PIM;
  if ((!(flag_radmod == 0)) && (flag_fermi == 0))
    P4_0_miss_2 =
        P4_Pini + P4_Eini_new - P4_E_prime_new - P4_PIP - P4_Pfin - P4_PIM;
  if ((!(flag_radmod == 0)) && (flag_fermi == 1))
    P4_0_miss_2 = P4_Pini_fermi + P4_Eini_new - P4_E_prime_new - P4_PIP -
                  P4_Pfin - P4_PIM;

  P4_pim_miss = P4_Pini + P4_Eini - P4_E_prime - P4_PIP - P4_Pfin;

  if ((flag_radmod == 0) && (flag_fermi == 1))
    P4_pim_miss_2 = P4_Pini_fermi + P4_Eini - P4_E_prime - P4_PIP - P4_Pfin;
  if ((!(flag_radmod == 0)) && (flag_fermi == 0))
    P4_pim_miss_2 = P4_Pini + P4_Eini_new - P4_E_prime_new - P4_PIP - P4_Pfin;
  if ((!(flag_radmod == 0)) && (flag_fermi == 1))
    P4_pim_miss_2 =
        P4_Pini_fermi + P4_Eini_new - P4_E_prime_new - P4_PIP - P4_Pfin;
