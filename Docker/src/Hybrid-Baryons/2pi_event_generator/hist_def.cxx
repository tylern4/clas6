...
int hist_def(Float_t E_beam) {
  ...
  h_zel = new TH1F("Z_EL", "Z_EL", 100, Targ_off - Targ_len / 2. - 1.,
                   Targ_off + Targ_len / 2. + 1.);
  ...
  h_dalitz = new TH2F("dalitz", "dalitz", 100, MPIM + MPIP, W_max - MP, 100,
                      MPIP + MP, W_max - MPIM);

  h_0_miss = new TH1F("h_0_miss", "h_0_miss", 1000, -0.05, 0.05);
  h_pim_miss = new TH1F("h_pim_miss", "h_pim_miss", 200, MPIM * MPIM - 0.05,
                        MPIM * MPIM + 0.05);
  h_0_miss_2 = new TH1F("h_0_miss_2", "h_0_miss_2", 1000, -0.05, 0.05);
  h_pim_miss_2 = new TH1F("h_pim_miss_2", "h_pim_miss_2", 200,
                          MPIM * MPIM - 0.05, MPIM * MPIM + 0.05);
  ...
  for (Short_t iy = 0; iy <= 36; iy++) {
    ...
    h_odn_inv_m12[iy] = new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100,
                                 MPIP + MPIM, 1.25 + 0.025 * iy - MP);
    qqq.str("");

    qqq << "h_odn_inv_m23_" << 10000 * (1.2375 + 0.025 * iy);
    h_odn_inv_m23[iy] = new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100,
                                 MPIP + MP, 1.25 + 0.025 * iy - MPIM);
    qqq.str("");

    qqq << "h_odn_alpha_" << 10000 * (1.2375 + 0.025 * iy);
    h_odn_alpha[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, 0, 2 * M_PI);
    qqq.str("");

    qqq << "h_odn_theta_" << 10000 * (1.2375 + 0.025 * iy);
    h_odn_theta[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, 0, M_PI);
    qqq.str("");
  ...
  for (Short_t iy = 0; iy < 18; iy++) {
    ...
    h_odn_wwide_inv_m12[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, MPIP + MPIM,
                 2.2125 + 0.05 * iy - MP);
    qqq.str("");

    qqq << "h_odn_wwide_inv_m23_" << 10000 * (2.1875 + 0.05 * iy);
    h_odn_wwide_inv_m23[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, MPIP + MP,
                 2.2125 + 0.05 * iy - MPIM);
    qqq.str("");

    qqq << "h_odn_wwide_alpha_" << 10000 * (2.1875 + 0.05 * iy);
    h_odn_wwide_alpha[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, 0, 2 * M_PI);
    qqq.str("");

    qqq << "h_odn_wwide_theta_" << 10000 * (2.1875 + 0.05 * iy);
    h_odn_wwide_theta[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, 0, M_PI);
    qqq.str("");

    qqq << "h_odn_wwide_theta_2_" << 10000 * (2.1875 + 0.05 * iy);
    h_odn_wwide_theta_2[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, 0, M_PI);
    qqq.str("");
  }
  ...
  for (Short_t iy = 0; iy < 15; iy++) {
    ...
    h_odn_wgt3_inv_m12[iy] = new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100,
                                      MPIP + MPIM, 3.1875 + 0.1 * iy - MP);
    qqq.str("");

    qqq << "h_odn_wgt3_inv_m23_" << 10000 * (3.1375 + 0.1 * iy);
    h_odn_wgt3_inv_m23[iy] = new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100,
                                      MPIP + MP, 3.1875 + 0.1 * iy - MPIM);
    qqq.str("");
  ...
  for (Short_t iy = 0; iy <= 11; iy++) {
    ...
    h_odn_w_dep_t[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, W_min, W_max);
    qqq.str("");
  ...
  for (Short_t iy = 0; iy <= 32; iy++) {
    ...
    h_odn_q2_dep_t[iy] =
        new TH1F(qqq.str().c_str(), qqq.str().c_str(), 100, Q2_min, Q2_max);
    qqq.str("");
  ...
  /*
  h_int_crsect_t[0] = new TH1F("h_int_crsect_t_0","h_int_crsect_t_0",71,
  1.25, 3.025); h_int_crsect_l[0] = new
  TH1F("h_int_crsect_l_0","h_int_crsect_l_0",71, 1.25, 3.025);

  h_int_crsect_t[26] = new
  TH1F("h_int_crsect_t_26","h_int_crsect_t_26",71, 1.25, 3.025);
  h_int_crsect_l[26] = new
  TH1F("h_int_crsect_l_26","h_int_crsect_l_26",71, 1.25, 3.025);

  for (Short_t iy=1; iy<=25; iy++) {

  qqq << "h_int_crsect_t_" <<iy;
  h_int_crsect_t[iy] = new TH1F(qqq.str().c_str(),qqq.str().c_str(),71,
  1.25, 3.025); qqq.str("");

  qqq << "h_int_crsect_l_" <<iy;
  h_int_crsect_l[iy] = new TH1F(qqq.str().c_str(),qqq.str().c_str(),71,
  1.25, 3.025); qqq.str("");
      }*/
}
