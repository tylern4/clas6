/** @file Skim_p_pi+_pi-.cc
 *  @brief Skims \f$\gamma p \rightarrow p \pi^+ \pi^-\f$ events from 
 *         compressed ROOT files and produces an output ROOT Event file.
 */
...
int main(int __argc,char *__argv[]){
  ...
  TTree *outTree = new TTree("T","p pi+ pi- skim"); // output tree
  ...
  for(int n_arg = optind; n_arg < __argc; n_arg++){
	...
	if(abs(mm) < 0.3){ // very loose missing mass cut (to save time)

	  // kinematic fit to gamma p --> p pi+ pi- 
	  clasevent.KinematicFit(); 

	  if(clasevent.Prob() > 0.01) {// confidence level cut
...
void PrintUsage(){
  ...
  cout << "Usage: Skim_p_pi+_pi- -o <file> inFile1 inFile2 ..." << endl;
  cout << endl;
  cout << "Options:" << endl;
  ...
  cout << "[thebride@kbill]$ Skim_p_pi+_pi- -o 43582_omega.root "
