 ...
 ********************************************************
 * 
 * created by:  Paul M Eugenio for CLAS
...
int printGampEvent(FILE *fp,struct particleMC_t *X,struct particleMC_t *Y, vector4_t *beam);
int printGampPart(FILE *fp,struct particleMC_t *Isobar);
/*
 ***********************
 *                     *
 *  PrintUsage()       *
...
int PrintUsage(char *processName)
{
  ...
  fprintf(stderr,"\t-r<runNo> default runNo is 9000(ascii format only). \n");
fprintf(stderr,"\t-L<low_xmasscut> Low X mass cut\n");
  fprintf(stderr,"\t-U<high_xmasscut> High X mass cut\n");

  fprintf(stderr,"\t-B<E0,E1> Generate Bremsstrahlung beam from E0 to E1 (over rides input file value) \n");
  ...
  fprintf(stderr,"\t-o<filename> Save in gamp format. \n");
...
main(int argc,char **argv)
{
  ...
  vector4_t beta,v4[2],initBeam4, initTarget4;
  ...
  double xmass_lowcut=0,xmass_highcut=0;
  double costheta,theta,phi,lf,lfmax,beam_pz;
  double bremLow=0, bremHigh=0;
  time_t now;
  ...
  if (argc == 1){
	...
	switch (*argptr) {
	case 'B':
          bremLow = atof(strtok(++argptr,","));
          bremHigh = atof(strtok(NULL,","));
          fprintf(stderr,"Generating brem from E0: %lf to E1: %lf \n",
                  bremLow, bremHigh);
          break;
	case 'L':
	  xmass_lowcut = atof(++argptr);
	  fprintf(stderr,"Cutting at low mass %lf\n",xmass_lowcut);
	  break;
	case 'U':
	  xmass_highcut = atof(++argptr);
	  fprintf(stderr,"Cutting at high mass %lf\n",xmass_highcut);
	  break;
	case 'd':
	  Debug =1;
	  break;
...
/* Fill particle information */
  ...
  initBeam4.space.x = atof(token);
  token=strtok(NULL," ");
  initBeam4.space.y = atof(token);
  token=strtok(NULL," ");
  initBeam4.space.z = atof(token);
  token=strtok(NULL," ");
  beam.mass =  atof(token);
  
  fprintf(stderr,"Reading: \tInitial beamp.x \tbeamp.y \tbeamp.z \tbeamMass\n");
  fprintf(stderr,"Found: \t\t%lf \t%lf \t%lf \t%lf \n",
	  initBeam4.space.x, initBeam4.space.y, 
	  initBeam4.space.z, beam.mass);  
  ...
  initTarget4.space.x = atof(token);
  token=strtok(NULL," ");
  initTarget4.space.y = atof(token);
  token=strtok(NULL," ");
  initTarget4.space.z = atof(token);
  token=strtok(NULL," ");
  target.mass =  atof(token);
  fprintf(stderr,"Reading: \tInit targetp.x \ttargetp.y \ttargetp.z \ttargetMass\n");
  fprintf(stderr,"Found: \t\t%lf \t%lf \t%lf \t%lf \n",
	  target.p.space.x, target.p.space.y, 
	  target.p.space.z, target.mass);  
  ...
  while(naccepted <max){
    ...
    // 
    // smear the beam for the next event
    //
    
    if( bremHigh ) //bremsstrahlung distribution
      initBeam4.space.z = bremLow * exp( randm(0.0, 1.0) * log( bremHigh/bremLow ));
 

    initBeam4.t = energy(beam.mass,&(initBeam4.space));
    beam.p.space.x = initBeam4.space.x;
    beam.p.space.y = initBeam4.space.y;
    beam.p.space.z = initBeam4.space.z;
    beam.p.t = energy(beam.mass,&(beam.p.space)); 
    
    //
    // set up the target particle 4vec
    //
    
    target.p.space.x = initTarget4.space.x;
    target.p.space.y = initTarget4.space.y;
    target.p.space.z = initTarget4.space.z;
    ...
    //
    // Boost beam and target to overall CM
    //
    v4[0]= beam.p;
    v4[1]= target.p;
    nv4=2;
    ...
    if(xmass>xmass_lowcut && xmass<xmass_highcut){
      if(Debug)
	fprintf(stderr,"xmass after cut: %lf\n",xmass);
	...
	if( (lfevents % 10) == 0 )
      ...
      else{ //select events via phase space
	...
	*/
	...
	if(lf > randm(0.0,lfmax) ){// a phase space event 
	  ...
	  if(WriteGamp){
	    printGampEvent(fout,X,Y,&initBeam4);
	  }else{
	  
	  //
	  // print Event Header
	  //
	  fprintf(fout,"%d %d %d ",runNo,naccepted, NFinalParts);
 ...
 * printGamp()
 *
 * Write to gamp pwa format
 *******************************/
int countGampParts(int *gpcount, struct particleMC_t *Isobar)
{
  int i;
  ...
  for(i=0;i<Isobar->nchildren;i++)
    countGampParts(gpcount,Isobar->child[i]);
  
}

int printGampEvent(FILE *fp,struct particleMC_t *XIsobar,
		   struct particleMC_t *YIsobar, vector4_t *beam)
{
  int i=0;
  
  countGampParts(&i,XIsobar);
  countGampParts(&i,YIsobar);
  fprintf(fp,"%d\n", i + 1);//parts + beam
  //print photon beam
  fprintf(fp,"1 0 %lf %lf %lf %lf\n",beam->space.x,beam->space.y,beam->space.z,beam->t );
  printGampPart(fp,YIsobar);
  printGampPart(fp,XIsobar);
}

int printGampPart(FILE *fp,struct particleMC_t *Isobar)
{
  int i;
  if ( Isobar->nchildren == 0 && Isobar->flag/10 == 1 )
    {
      fprintf(fp,"%d %d ",Isobar->particleID,Isobar->charge);
	...
	fprintf(fp,"%d %d ",Isobar->child[i]->particleID,Isobar->child[i]->charge);
	fprintf(fp,"%lf %lf %lf %lf\n",
		Isobar->child[i]->p.space.x,
		Isobar->child[i]->p.space.y,
		Isobar->child[i]->p.space.z,
		Isobar->child[i]->p.t);
      }
      *************************/
      printGampPart(fp,Isobar->child[i]);
    } 
}
